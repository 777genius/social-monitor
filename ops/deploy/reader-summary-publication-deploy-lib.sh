...
if declare -F source_reviewed_deploy_library >/dev/null && \
  ...
  # shellcheck source=ops/deploy/reader-summary-publication-catalog-query-lib.sh
  source "$reader_summary_publication_support_dir/reader-summary-publication-catalog-query-lib.sh"
...
reader_summary_publication_prebootstrap_target_daily_runner || \
  fail 'target publication prebootstrap daily-runner image reconstruction failed'
...
reader_summary_feed_promotion_index_state() {
  local secret=$1 ca_certificate=$2 output
  output=$("${COMPOSE[@]}" --profile app run -T --rm --no-deps \
    --user 0:0 \
    -v "$secret:/run/secrets/reader-summary-publication-admin-url:ro" \
    -v "$ca_certificate:/run/social-monitor-db/ca-certificate.crt:ro" \
    migrate sh -c '
      ...
      DATABASE_URL=$(cat /run/secrets/reader-summary-publication-admin-url)
      export DATABASE_URL
      npm run check:feed-promotion-index-recovery -- inspect
    ') || return
  case $output in
    *feed_promotion_indexes=all_valid*) printf 'all_valid\n' ;;
    *feed_promotion_indexes=pending*) printf 'pending\n' ;;
    *) return 1 ;;
  esac
}
...
reader_summary_publication_run_postgres_client() (
  ...
  if [[ $operation == bootstrap ]]; then
    ...
    if [[ $sql == */reader-summary-publication-pre-migration.sql ]]; then
      local ownership_sql=${sql%/*}/reader-summary-publication-tenant-ownership.sql
...
# The large catalog query lives in reader-summary-publication-catalog-query-lib.sh.
