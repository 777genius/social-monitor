...
[[ ${#bridge_paths[@]} == 23 ]] || fail 'daily C1 bridge release manifest count drifted'
[[ $(printf '%s\n' "${bridge_paths[@]}" | LC_ALL=C sort -u | wc -l | tr -d ' ') == 23 ]] || \
