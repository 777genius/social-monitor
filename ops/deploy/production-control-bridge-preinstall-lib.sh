...
deploy_control_production_bridge_exact_installed_file() {
  ...
  production_control_bridge_require_replaceable_destination "$installed" "$label"
...
production_control_bridge_require_replaceable_destination() {
  local destination=$1 label=$2
  if [[ -e $destination || -L $destination ]]; then
    [[ -f $destination && ! -L $destination ]] || \
      fail "$label destination is not an ordinary regular file"
...
production_control_bridge_install_descriptor() {
  ...
  production_control_bridge_require_replaceable_destination "$destination" "$label"
...
production_control_bridge_validate_phase_state() {
  ...
  production_control_bridge_require_replaceable_destination \
    "$CONTROL/github-production-deploy.sh" 'deploy entrypoint'
  production_control_bridge_require_replaceable_destination \
    "$CONTROL/github-production-deploy-wrapper.sh" 'deploy wrapper'
  production_control_bridge_require_replaceable_destination \
    "$CONTROL/refresh-codex-auth.sh" 'authentication refresh helper'
  production_control_bridge_require_replaceable_destination \
    "$CONTROL/x-collector.Dockerfile" 'X collector Dockerfile'
