...
DEPLOY_CONTROL_DAILY_FINAL_ANCHOR=$anchor
...
git -C "$REPO" checkout -qb final-base "$anchor"
printf 'runtime\n' > "$REPO/runtime"
...
git -C "$REPO" merge -qm final "$bridge"
...
deploy_control_is_reviewed_daily_final_transition "$bridge" "$target" || \
  fail 'reviewed bridge merge was rejected'
...
if deploy_control_is_reviewed_daily_final_transition "$bridge" "$drift"; then
  fail 'non-merge target was admitted'
fi
