...
grep -F 'verify_live_postgres_admission "$postgres_env"' "$ENTRYPOINT" >/dev/null; grep -F 'reader-summary-weekly-run|reader-summary-production-history|reader-summary-daily-terminal-set-receipt-v1|' "$ENTRYPOINT" >/dev/null
