...
cp "$SCRIPT_DIR/docker-maintenance-lib.sh" "$SCRIPT_DIR/production-transition-marker-lib.sh" "$REPO/ops/deploy/"
...
run_entrypoint() {
  SOCIAL_MONITOR_DEPLOY_TEST_MODE=1 PRODUCTION_FORWARD_TEST_ALLOW_LOCAL_ORIGIN=1 \
...
run_backup_fixture() {
  ...
  BACKUP_DATABASE_URL="$backup_url" BACKUP_API_DATABASE_URL="$BACKUP_API_DATABASE_URL" SOCIAL_MONITOR_DEPLOY_TEST_MODE=1 PRODUCTION_FORWARD_TEST_ALLOW_LOCAL_ORIGIN=1 \
...
ENTRYPOINT=$ENTRYPOINT ORDINARY_RENDER_LOG=$ORDINARY_RENDER_LOG \
  SOCIAL_MONITOR_DEPLOY_TEST_MODE=1 PRODUCTION_FORWARD_TEST_ALLOW_LOCAL_ORIGIN=1 SOCIAL_MONITOR_DEPLOY_ROOT=$ROOT \
...
git -C "$REPO" update-ref refs/remotes/origin/main "$TARGET_SHA"; git -C "$REPO" checkout -q "$TARGET_SHA"; run_entrypoint upload "$TARGET_SHA" < "$FIXTURE/frontend.tgz" >/dev/null
