...
production_control_bridge_validate_control_destinations() {
  local destination label
  while IFS='|' read -r destination label; do
...
$PRODUCTION_CONTROL_BRIDGE_CONTROL/github-production-deploy.sh|deploy entrypoint
$PRODUCTION_CONTROL_BRIDGE_CONTROL/github-production-deploy-wrapper.sh|deploy wrapper
$PRODUCTION_CONTROL_BRIDGE_CONTROL/refresh-codex-auth.sh|authentication refresh helper
$PRODUCTION_CONTROL_BRIDGE_CONTROL/x-collector.Dockerfile|X collector Dockerfile
EOF
}
