...
FINAL_SOURCE_SHA=3dfbb94ae7f0994c99ce933fac78853185905e74
...
verify_bridge_candidate() {
  ...
  [[ $parent == "$FINAL_SOURCE_SHA" ]] || \
    bridge_fail 'candidate parent is not the exact 3dfbb94 source commit'
...
PY
  ...
  FINAL_SHA=$(printf 'synthetic restore of exact 3dfbb94 tree\n' | \
    GIT_AUTHOR_NAME='Runtime Bridge Test' \
    GIT_AUTHOR_EMAIL=runtime-bridge@example.invalid \
    GIT_COMMITTER_NAME='Runtime Bridge Test' \
    GIT_COMMITTER_EMAIL=runtime-bridge@example.invalid \
...
run_transition_test() {
  ...
  [[ $(git -C "$object_repository" rev-parse "$FINAL_SHA^{tree}") == \
    ...
    bridge_fail 'synthetic final target does not restore the exact 3dfbb94 tree'
...
PY
  ...
  printf 'Exact 3dfbb94/eb9 runtime-control refresh B-to-F transition tests passed\n'
}
