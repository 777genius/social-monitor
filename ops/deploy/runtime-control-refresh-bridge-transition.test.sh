...
verify_bridge_candidate() {
  ...
  [[ $parent == "$FINAL_SOURCE_SHA" ]] || \
    bridge_fail 'candidate parent is not the exact merged badd source commit'
  ...
  for line in "${manifest_lines[@]}"; do
    ...
    case $path in
      ...
      ops/deploy/runtime-control-refresh-bridge-transition.test.sh)
        ;;
      *)
        assert_same_entry "$repository" "$candidate" "$CONTROL_MARKER_SHA" "$path"
        ((rejected_count += 1))
        ;;
    esac
  done
  ((count == 191 && rejected_count == 188)) || \
    bridge_fail 'bridge manifest partition is not exactly 188 control-marker paths plus 3 gate paths'
...
create_synthetic_targets() {
  ...
  git clone -q --shared --no-checkout "$PROJECT_ROOT" "$object_repository"
...
PY
  ...
  FINAL_SHA=$(printf 'synthetic restore of exact badd tree\n' | \
    GIT_AUTHOR_NAME='Runtime Bridge Test' \
    GIT_AUTHOR_EMAIL=runtime-bridge@example.invalid \
    GIT_COMMITTER_NAME='Runtime Bridge Test' \
    GIT_COMMITTER_EMAIL=runtime-bridge@example.invalid \
...
run_transition_test() {
  ...
  [[ $(git -C "$object_repository" rev-parse "$FINAL_SHA^{tree}") == \
    ...
    bridge_fail 'synthetic final target does not restore the exact badd tree'
...
PY
  ...
  printf 'Exact badd/d7 runtime-control refresh B-to-F transition tests passed\n'
}
...
case ${1:-} in
  --verify-candidate)
    ...
    printf 'bridge-plan backend=false frontend=false control=true runtime=false x=false\n'
    ;;
  '')
