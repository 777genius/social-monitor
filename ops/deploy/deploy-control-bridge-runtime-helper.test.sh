...
[[ $production_actual_delta == "$production_expected_delta" ]] || \
  fail 'production final differs from desired base beyond the three preserved files'
while IFS= read -r path; do
...
git clone -q --no-checkout "$SOURCE_REPO" "$production_preflight_repo"
...
# The exact production tree matcher is exercised above. Keep the state-matrix
# fixtures focused on pre-mutation host state instead of rebuilding that large
# synthetic tree for every negative case.
...
[[ -e $production_preflight_side_effect ]] || \
  fail 'valid production bridge preflight did not reach the lifecycle mutation boundary'
...
for drift_path in \
  ...
  "$DEPLOY_CONTROL_PRODUCTION_RABBITMQ_TEST_PATH"; do
...
printf '%s\n' \
  ...
  'final_tree=desired_base_plus_three_preserved_files'
