...
if [[ -n $(git -C "$SOURCE_REPO" status --porcelain=v1 --untracked-files=all) ]]; then
  if ! git -C "$SOURCE_REPO" diff --quiet HEAD --; then
    git -C "$SOURCE_REPO" diff --binary HEAD -- | \
      git -C "$PRODUCTION_SOURCE_REPO" apply
  fi
  while IFS= read -r untracked; do
    install -d "$PRODUCTION_SOURCE_REPO/$(dirname "$untracked")"
    cp "$SOURCE_REPO/$untracked" "$PRODUCTION_SOURCE_REPO/$untracked"
  done < <(git -C "$SOURCE_REPO" ls-files --others --exclude-standard)
  git -C "$PRODUCTION_SOURCE_REPO" add -A
  git -C "$PRODUCTION_SOURCE_REPO" commit -qm \
    'test: synthesize current production bridge sources'
fi
...
production_reviewed_source=$(git -C "$REPO" rev-parse HEAD)
production_squashed_tree=$(
  deploy_control_production_bridge_expected_tree "$production_reviewed_source"
) || fail 'exact squashed production bridge tree could not be built'
production_squashed_bridge=$(production_single_parent_commit \
  "$production_squashed_tree" "$DEPLOY_CONTROL_PRODUCTION_BRIDGE_BASE" \
  'test: synthesize exact squashed production bridge')
production_synthetic_final=$(production_target_for_bridge \
  "$production_squashed_bridge")
release_tip=$(production_checkout_release_tip "$production_synthetic_final") || \
  fail 'synthetic final is neither a direct release commit nor a two-parent PR merge'
production_locate_release_pair "$release_tip" || \
  fail 'release lineage does not begin with the exact bridge and final pair'
[[ $production_bridge == "$production_squashed_bridge" && \
   $production_target == "$production_synthetic_final" && \
   $(git -C "$REPO" rev-parse "$production_bridge^") == \
     "$DEPLOY_CONTROL_PRODUCTION_BRIDGE_BASE" && \
   $(git -C "$REPO" rev-parse "$production_target^") == \
     "$production_bridge" ]] || \
  fail 'synthetic final did not preserve the exact base/bridge/final lineage'
...
production_pr_merge=$(production_merge_commit \
  ...
  "$DEPLOY_CONTROL_PRODUCTION_BRIDGE_BASE" "$production_valid_target" \
  'test: synthesize production final PR merge')
...
[[ $production_pr_head == "$production_valid_target" ]] || \
...
# Direct deploy SHA entrypoints must fetch an object that is genuinely absent
# from the local repository before candidate-specific preflight. The real
# preflight then rejects malformed host state without reaching atomic repair.
production_absent_bridge=$(printf '%s\n' \
  'test: exact bridge created after preflight clone' | \
  git -C "$PRODUCTION_SOURCE_REPO" commit-tree \
    "$production_squashed_tree" \
    -p "$DEPLOY_CONTROL_PRODUCTION_BRIDGE_BASE")
if git -C "$production_preflight_repo" cat-file -e \
    "$production_absent_bridge^{commit}" 2>/dev/null; then
  fail 'absent-local-object fixture was already present before fetch'
fi
production_reset_preflight_state
printf 'malformed\n' > "$STATE/backend.sha"
production_admission_gap_side_effect=$FIXTURE/production-admission-gap-side-effect
production_deploy_lock=$FIXTURE/production-preflight-deploy.lock
production_admission_lock=$FIXTURE/production-preflight-admission.lock
rm -f "$production_admission_gap_side_effect"
if (trap - EXIT
    export DEPLOY_LOCK=$production_deploy_lock
    export POSTGRES_ADMISSION_LOCK=$production_admission_lock
    source "$SCRIPT_DIR/deploy-control-lib.sh"
    source "$SCRIPT_DIR/deploy-control-bridge-lib.sh"
    load_deploy_control_bridge_library() { :; }
    acquire_postgres_admission_with_daily_priority() { flock -w 10 "$1"; }
    fetch_main() {
      git -C "$REPO" fetch --quiet "$PRODUCTION_SOURCE_REPO" \
        "$production_absent_bridge"
    }
    validate_main_commit() {
      [[ $1 == "$production_absent_bridge" ]] && \
        git -C "$REPO" cat-file -e "$1^{commit}"
    }
    postgres_pool_atomic_legacy_state() {
      : > "$production_admission_gap_side_effect"
      return 0
    }
    deploy_release "$production_absent_bridge") >/dev/null 2>&1; then
  fail 'post-fetch bridge preflight admitted malformed host state'
fi
[[ ! -e $production_admission_gap_side_effect ]] && \
  git -C "$production_preflight_repo" cat-file -e \
    "$production_absent_bridge^{commit}" || \
  fail 'direct SHA deploy did not fetch and fail closed before atomic repair'
