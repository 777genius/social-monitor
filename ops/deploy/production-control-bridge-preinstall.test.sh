...
if ! git -C "$PROJECT_REPO" diff --quiet HEAD --; then
  ...
  remediation_tree=$(git -C "$SOURCE_REPO" rev-parse 'HEAD^{tree}')
  remediation_target=$(printf '%s\n' \
    'test: squash production bridge remediation onto reviewed base' | \
...
for destination_name in \
  ...
  grep -F "destination is a symbolic link" <<< "$symlink_error" >/dev/null || \
