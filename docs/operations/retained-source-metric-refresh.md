...
Initial planning emits every retained HN/Reddit source ID in the admitted window, including sources with zero visible feed projections or below promotion floors. It also emits rejected entries for review. Any rejected target blocks apply. There is no top-16 selection, source-ID override, force switch, fresh output directory, provider retry switch, or bypass for an existing operation.

Once `operation.json` exists, its exact original IDs are the authorized membership. Prepare, commit (including exact replay), apply and resume reread all those IDs within the fixed tenant/workspace. This query deliberately has no creation-time, publication-window or provider filter: an original that moves or becomes ineligible must fail validation, never disappear through filtering. Missing/replaced originals and scope, provider, URL, binding, config, feed, hidden/disabled drift still refuse. Initial planning continues to scan the whole window. Per-batch, post-fetch and Serializable sample checks use the same original IDs and effective identity digests.

Later arrivals cannot join original batches, consume their budget or appear in original results. Existing-operation dry run separately scans the full window and emits `inventoryAudit.currentWindowTargetCount` and `outsideOperationSourceItemIds`; its `currentTargets`, when drift is present, means current original-ID inventory. These separate scans are diagnostic observations, not an atomic snapshot. Amendment `inventory` and its SHA cover all original IDs, not the growing global window. Parent accounts for outside-operation arrivals separately without resetting this operation.
...
The later supplied diagnostic reports 3,329 original targets, 3,333 current window rows, no missing originals, four late arrivals and exactly 19 original `identityDigest` changes, with no other field drift or rejections. The 23-row timestamp intake shows all four arrivals created after original `plannedAt` `2026-09-06T02:28:41.454Z`; all 19 changed originals were created before planning and content-updated afterward. This supersedes the earlier one-row diagnostic. An opaque digest does not reconstruct old content. Review the exact fresh 19-row proposal SHA; the diagnostic is not itself an amendment. The amendment replaces only explicitly reviewed identity digests; scope, all 3,329 IDs, original ordering/batches, config/feed identities, cutoff, source base, limits, plannedAt and original authority baselines stay fixed. Fresh authority counters/times are captured separately, and may change again without becoming identity drift.

Use these additional arguments with the wrapper and common identity/release arguments above:

```sh
# 1. Capture every original ID's current inventory and explicit digest diff. No provider/projection work.
...
Preparation prints `result.value` and `amendmentSha`; commit prints the verified original/effective head. Capture and commit each acquire a full fresh original-ID inventory. Both make zero OAuth/provider/projection calls. Apply checks that full inventory once inside the fenced use case; the duplicate CLI apply scan was removed. Dry run additionally scans the complete current window for audit, and per-batch, post-fetch and Serializable projection guards remain.
...
Only 8 proposals, 8 committed amendments and 32 changed identity digests per proposal are admitted; each record is at most 16 MiB and JSON depth 32. The canonical `metricReviewedContentChangeLimit` is an administrative review ceiling, increased from 16 to admit the observed 19 versions; quality/rating thresholds and provider limits are unchanged. Stored v1 amendments with at most 16 changes remain readable. These are ceilings, not authorization to accept extra incident changes. Review every changed digest in the exact proposal SHA; additional drift requires a fresh separate review. No automatic amendment loop exists. Uncommitted proposals remain immutable and count against the bound. A newer proposal captures all earlier proposals; installing another proposal invalidates an earlier proposal's directory proof. Exact latest committed replay rechecks original inventory and is idempotent before budget; stale parent/fork/gap/unequal bytes or exhausted limits require reconciliation without resets.
