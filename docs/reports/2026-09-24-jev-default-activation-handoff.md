...
- Without `READER_VALUE_DISCOVERY_SCOPES`, the worker walks currently enabled interests that have an enabled source binding and visible feed items, one interest and at most 100 items per tick. The interest cursor advances only after that scope's discovery succeeds, so a large interest cannot permanently starve another. An explicit scope list still bounds the rollout when supplied. Compose forwards the optional scope and backfill overrides to API and worker only when provided.
