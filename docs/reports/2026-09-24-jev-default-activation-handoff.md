...
- Without `READER_VALUE_DISCOVERY_SCOPES`, the worker walks currently enabled interests that have an enabled source binding and visible feed items, one interest and at most 100 items per tick. The interest cursor advances after success or a recoverable discovery/storage failure; a failed interest is retried on the next rotation without starving other interests or assessment maintenance. A global configuration failure does not advance the cursor. An explicit scope list still bounds the rollout when supplied. Compose forwards the optional scope and backfill overrides to API and worker only when provided.
