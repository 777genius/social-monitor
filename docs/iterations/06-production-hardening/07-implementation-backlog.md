...
- PR 13 code quality guardrails added in current branch: every feature use case now has a focused sibling spec, `npm run check:code-quality` blocks missing specs, thrown `DomainError` from features, direct in-memory adapter construction in features, unscoped REST controllers and production `console.*`, and release evidence now requires this gate.
- PR 14 code quality guardrails strengthened in current branch: feature specs must reference their exported use case and contain executable tests, and committed `.only`/`.skip` tests are blocked.
