...
- PR 15 release gate coverage strengthened in this quality pass: `npm run check:release` now fails if any blocking release gate is not wired into `npm run verify`.
- PR 16 API-key workspace authorization added in this implementation pass: API key create/list/revoke REST actions now require owner/admin workspace role through an Identity authorization policy port, with e2e coverage for missing/insufficient role denial.
