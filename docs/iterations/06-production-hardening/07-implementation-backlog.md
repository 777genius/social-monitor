...
- PR 11 worker shutdown drain added in commit `6978210`: platform worker runtime now stops accepting new work during shutdown, tracks active operations, waits for drain with an MVP timeout, and ingestion scan command handling rejects late work with `operation.backpressure` while allowing in-flight scans to finish lease cleanup.
- PR 11 scan DLQ inspection added in commit `e8a91cc`: ingestion now exposes a tenant-scoped support-safe scan dead-letter list through a feature/use-case port, REST endpoint and OpenAPI snapshot, returning failure class/operator action/correlation IDs without raw provider failure text.
- PR 12 retention contract gate added in commit `208abc9`: `ops/privacy/retention-contract.json` assigns retention/export/delete/legal-hold policy to every Prisma table, `npm run check:retention` blocks schema drift without table policy coverage, and the release evidence gate now requires this retention control before beta promotion.
- PR 12 retention purge dry-run planner added in commit `24484a1`: new Privacy context loads the retention contract through a port/adapter and produces deterministic purge plans with legal-hold behavior, retained-table entries and cutoff timestamps, verified by `npm run check:retention-plan`.
- PR 13 code quality guardrails added in commit `83e8f48`: every feature use case now has a focused sibling spec, `npm run check:code-quality` blocks missing specs, thrown `DomainError` from features, direct in-memory adapter construction in features, unscoped REST controllers and production `console.*`, and release evidence now requires this gate.
- PR 14 code quality guardrails strengthened in commit `53f6dbd`: feature specs must reference their exported use case and contain executable tests, and committed `.only`/`.skip` tests are blocked.
...
- PR 19 source binding workspace authorization added in commit `53d5457`: Monitoring source binding creation now requires owner/admin workspace role before binding provider config or writing audit events, with OpenAPI contract coverage and HTTP e2e/smoke coverage for missing role, viewer denial and owner success.
- PR 20 scan policy workspace authorization added in commit `2db3fcb`: Monitoring scan policy changes now require owner/admin workspace role before changing scan cadence or writing audit events, with OpenAPI contract coverage and HTTP e2e/smoke coverage for missing role, viewer denial and owner success.
- PR 21 manual scan request workspace authorization added in commit `844d45c`: Monitoring manual scan requests now require owner/admin/member workspace role before quota reservation, queue enqueue or audit events, with OpenAPI contract coverage and HTTP e2e/smoke coverage for missing role, viewer denial and member success.
- PR 22 summary request workspace authorization added in commit `4e95e93`: Summary request creation now requires owner/admin/member workspace role before quota reservation or summary job creation, with OpenAPI contract coverage and HTTP e2e/smoke coverage for missing role, viewer denial and member success.
- PR 23 summary regeneration workspace authorization added in commit `d8836e5`: Summary regeneration now requires owner/admin/member workspace role before quota reservation or regeneration job creation, with OpenAPI contract coverage and HTTP e2e/smoke coverage for missing role, viewer denial and member success.
- PR 24 webhook endpoint write workspace authorization added in commit `0e40f5c`: Delivery webhook endpoint creation and disable now require owner/admin workspace role after API-key scope verification, with OpenAPI contract coverage and HTTP e2e/smoke coverage for missing role, viewer denial and owner/admin success.
- PR 25 webhook endpoint read workspace authorization added in commit `7c5b3cd`: Delivery webhook endpoint list/get now require `read:webhook_endpoints` API-key scope and any workspace role before read use cases or audit events, with OpenAPI contract coverage and HTTP e2e/smoke coverage for missing role, write-only scope denial and viewer read success.
- PR 26 summary read workspace authorization added in commit `a4906af`: Summary list/get now require any workspace role before read use cases, with OpenAPI contract coverage and HTTP e2e/smoke coverage for missing role denial and viewer read success.
- PR 27 feed read workspace authorization added in commit `e2eae16`: Feed list/get now require any workspace role before read use cases, with OpenAPI contract coverage and HTTP e2e/smoke coverage for missing role denial and viewer read success.
- PR 28 scan status workspace authorization added in commit `7bd42d4`: Scan job status reads now require any workspace role before status use case execution, with OpenAPI contract coverage and HTTP e2e/smoke coverage for missing role denial and viewer status success.
- PR 29 summary job status workspace authorization added in commit `9bba42f`: Summary job status reads now require any workspace role before status use case execution, with OpenAPI contract coverage and HTTP e2e/smoke coverage for missing role denial and viewer requested/completed status success.
- PR 30 delivery attempt read workspace authorization added in commit `fae50ea`: Delivery attempt status reads now require any workspace role before read use case execution, with OpenAPI contract coverage and HTTP e2e/smoke coverage for missing role denial and viewer status success.
- PR 31 digest read workspace authorization added in commit `f108390`: Delivery digest reads now require any workspace role before read use case execution, with OpenAPI contract coverage and HTTP e2e/smoke coverage for missing role denial and viewer digest success.
- PR 32 realtime event read workspace authorization added in commit `cff66cf`: Realtime event REST replay now requires any workspace role before read use case execution, with OpenAPI contract coverage and HTTP e2e/smoke coverage for missing role denial and viewer replay success.
- PR 33 scan dead-letter read workspace authorization added in commit `833f3cc`: Support-safe scan dead-letter inspection now requires owner/admin workspace role before read use case execution, with OpenAPI contract coverage and HTTP e2e/smoke coverage for missing role denial, viewer denial and admin success.
- PR 34 event tenant-context contract added in commit `a8aa523`: Event catalog entries now declare tenant-scoped envelope/payload context, `check:events` blocks missing tenant/workspace/correlation contract fields, and summary-to-realtime projection carries tenant/workspace in replay payloads with AppModule e2e smoke coverage.
- PR 35 scan-to-feed API visibility evidence added in commit `f8ae341`: API-to-ingestion e2e now shares the feed read repository across the local single-process API/worker composition and asserts that executed fake-source scans are visible through REST `/feed/items`.
