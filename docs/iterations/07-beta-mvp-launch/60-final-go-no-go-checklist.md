...
- Ingestion/feed Prisma persistence foundation plus feed API and ingestion-worker runtime selectors pass `npm run check:ingestion-feed-persistence`.
...
14. ingestion/feed persistence smoke for source items, cursors, feed read models, feed runtime selector and ingestion-worker runtime selector, including `npm run check:ingestion-feed-persistence`
