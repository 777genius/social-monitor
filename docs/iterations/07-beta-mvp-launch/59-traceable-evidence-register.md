...
- `npm run check:ingestion-feed-persistence` proves the first ingestion/feed Prisma adapter foundation plus feed API and ingestion-worker runtime selectors: provider-level source item dedupe, scan cursor roundtrip, feed canonical URL dedupe, feed search/list, feed item rehydration, `FEED_PERSISTENCE=prisma` validation and `INGESTION_WORKER_PERSISTENCE=prisma` validation.
