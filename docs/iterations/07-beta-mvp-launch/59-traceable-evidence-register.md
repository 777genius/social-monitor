...
- `npm run check:ingestion-feed-persistence` proves the first ingestion/feed Prisma adapter foundation and feed runtime selector: provider-level source item dedupe, scan cursor roundtrip, feed canonical URL dedupe, feed search/list, feed item rehydration, and `FEED_PERSISTENCE=prisma` validation.
