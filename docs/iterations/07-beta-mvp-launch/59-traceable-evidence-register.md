...
- Monitoring scan command publishing now has a broker-backed runtime path through `AmqplibRabbitMqChannel` and `RabbitMqQueuePublisher`; full external beta still requires the worker-side RabbitMQ consumer/drain path.
