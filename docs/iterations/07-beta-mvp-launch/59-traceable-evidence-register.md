...
- Scan command transport now has broker-backed runtime paths on both sides: monitoring can publish through `AmqplibRabbitMqChannel`/`RabbitMqQueuePublisher`, and ingestion worker can drain commands through `RabbitMqScanCommandQueueReader` with ack/nack delivery semantics. Full external beta still requires outbox/inbox rollout and cross-process scheduler deployment evidence.
