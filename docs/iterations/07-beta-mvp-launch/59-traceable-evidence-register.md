...
- `npm run check:summary-persistence` proves the first summary Prisma adapter foundation: summary job idempotency/status transitions, completed/no-signal artifact JSON payload rehydration, paginated artifact list/read behavior and feedback idempotency/evidence roundtrip.
