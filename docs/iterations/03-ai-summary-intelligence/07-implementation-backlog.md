...
- PR 39 interest-scoped feed evidence added: scan commands now carry `interestId` from Monitoring to Ingestion, feed items persist it, feed list/read DTOs expose it and Summary evidence selection filters by interest. Feed canonical dedupe is tenant/workspace/interest scoped so the same URL can safely appear in different interests.
- PR 40 citation validation added: provider drafts are checked against the frozen evidence window before artifact persistence/event publication, and the eval harness uses the same validator to block invalid citation regressions.
- PR 41 summary eval output gate added: deterministic eval fixtures now produce a committed JSON report, `check:summary-evals` blocks stale/failing eval output and the MVP release contract requires this gate.
- PR 42 summary cost attribution gate added: deterministic eval fixtures now produce `ops/cost/summary-cost-attribution.json`, with tenant/workspace/topic/window/provider/model/prompt/schema attribution and a blocking `check:summary-cost` release gate.
- PR 43 summary window freshness gate added: summary read models now expose `freshness`, same-topic newer feed evidence marks an existing summary stale without mutating its artifact text, and `check:summary-window` blocks window/freshness regressions.
- PR 44 failed summary retry gate added: failed summary jobs can be replayed safely through the same job id/idempotency key, transient failures do not publish artifacts/events and `check:summary-retry` blocks retry regressions.
