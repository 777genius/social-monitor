# 207. Interest DSL Compiler Safety

## Status

Locked for interest/rules baseline.
...
Interest rules are user-defined policy/query artifacts. Compile them into validated provider-neutral query plans before execution.
...
Every interest rule update creates a new version. Scans and summaries record the rule version used.

## Best-Fact Choice

Do not execute user interest rules directly against providers. A compiler layer is needed for safety, cost control, provider portability and explainability.
