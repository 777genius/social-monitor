...
1. Turn usage comes from `tokenUsage.last` and replaces, rather than accumulates
...
- regression proof: the same contract suite fails on the previously vendored
  `main.30` and `main.40` artifacts and passes on `main.41`. On `main.40` a turn
  whose exact usage was 5 tokens was billed 1500.
