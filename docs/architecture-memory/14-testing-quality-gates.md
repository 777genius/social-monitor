...
- keep monitoring read REST management covered by `npm run check:monitoring-read-rest`, including topic pagination, source binding pagination, scan policy readback, workspace role authorization and safe protected config previews.
...
- keep notification preference REST management covered by `npm run check:notification-preference-rest`, including suppress/allow routing, workspace role authorization and immediate send-time reader consistency without requiring the full API gateway e2e suite on every inner-loop change.
