...
import { ActivateTopicSourceUseCase } from './activate-topic-source.use-case';

describe('ActivateTopicSourceUseCase', () => {
  it('activates legacy X target input as canonical x-twitter daily source', async () => {
    ...
    expect(workflows.createTopic.commands).toEqual([
  ...
  it('maps X account targets to search queries without leaking account_feed mode', async () => {
    ...
    expect(result.ok).toBe(true);
    expect(workflows.createTopic.commands[0]).toEqual(expect.objectContaining({
      name: '@openai',
      query: 'from:openai',
    }));
  ...
  it('does not create the user subscription when monitoring binding fails', async () => {
    ...
    expect(result.ok).toBe(false);
    expect(workflows.createTopic.commands).toHaveLength(1);
...
const command = (
  overrides: Partial<ActivateTopicSourceCommand> = {},
): ActivateTopicSourceCommand => ({
...
const recordingWorkflows = () => ({
  ...
  createTopic: {
    ...
    async execute(command: unknown) {
      ...
      return ok({ topicId: 'topic-1', created: true });
    },
  },
  bindSource: {
    commands: [] as unknown[],
