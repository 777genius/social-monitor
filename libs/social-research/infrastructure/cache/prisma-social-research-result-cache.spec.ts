...
describe("PrismaSocialResearchResultCache", () => {
  it("stores search runs per tenant/workspace scope and restores item dates", async () => {
    ...
    await expect(
      cache.readSearch("cache-key", {
        tenantId: tenantId("00000000-0000-7000-8000-000000000099"),
        workspaceId: workspaceId("00000000-0000-7000-8000-000000000020"),
      }),
    ).resolves.toBeNull();
    expect(
      client.databaseAccesses.every((access) => access?.kind === "tenant"),
    ).toBe(true);
  ...
  it("returns null for expired entries and deletes them", async () => {
    ...
    await client.setExpiresAt(
      "search",
      "cache-key",
      new Date("2026-07-04T11:59:59.000Z"),
    );
...
class FakePrismaSocialResearchCacheClient implements PrismaSocialResearchResultCacheClient {
  private readonly entries = new Map<string, StoredEntry>();
  readonly databaseAccesses: (DatabaseAccess | undefined)[] = [];
