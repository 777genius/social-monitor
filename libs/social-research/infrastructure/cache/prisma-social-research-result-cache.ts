...
export class PrismaSocialResearchResultCache
  implements SocialResearchResultCachePort
{
  ...
  private async read<TValue>(
    ...
    if (toDate(record.expires_at).getTime() <= this.options.clock.now().getTime()) {
  ...
  private async write(
    ...
    const payload = JSON.stringify(encodeSocialResearchResultCachePayload(value));
  ...
  private async deleteEntry(
    ...
    await withPrismaWriteRetry(() => this.prisma.$executeRaw`
