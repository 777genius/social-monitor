...
export class InterestCoveragePlanDraftDto implements InterestCoveragePlanDraft {
  ...
  @ApiProperty({ enum: ['ready', 'needs_input', 'already_bound', 'unsupported'] })
...
export const normalizePlanInterestCoverageRequest = (
  ...
  'keywords' | 'subreddits' | 'rssFeedUrls' | 'includeProviders' | 'excludeProviders'
> & {
...
export const normalizePlanDraftConfig = (config: Readonly<Record<string, unknown>>): SourceBindingConfig =>
  normalizeSourceBindingConfig(config);
...
const normalizeOptionalStringArray = (
  field: 'keywords' | 'subreddits' | 'rssFeedUrls' | 'includeProviders' | 'excludeProviders',
