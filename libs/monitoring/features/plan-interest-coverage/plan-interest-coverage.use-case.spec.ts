...
describe("PlanInterestCoverageUseCase", () => {
  it("plans executable Reddit scan passes from keyword and subreddit hints", async () => {
    ...
    const useCase = new PlanInterestCoverageUseCase(
      interests,
    ...
    expect(redditDraft?.sourceBindingDraft?.config.scanPasses).toEqual([
      expect.objectContaining({
        mode: "search",
        query: expect.stringContaining('"LLM monitoring"'),
    ...
    expect(redditDraft?.applyTarget?.path).toBe(
      "/interests/interest-1/source-bindings",
    );
  });

  it("applies source pack provider starters across Reddit, HN, GitHub and RSS", async () => {
    const interests = new FakeInterests();
    interests.add(makeInterest());
    const useCase = new PlanInterestCoverageUseCase(
      interests,
    ...
    expect(result.value.sourcePack).toMatchObject({
      key: "ai_dev",
      providerStarters: expect.arrayContaining([
        expect.objectContaining({
          providerKey: "reddit",
          subreddits: expect.arrayContaining(["LocalLLaMA"]),
        }),
        expect.objectContaining({
          providerKey: "hacker-news",
          queries: expect.arrayContaining(["LLM evals"]),
        }),
        expect.objectContaining({
          providerKey: "github-repo-radar",
          topics: expect.arrayContaining(["agents"]),
        }),
        expect.objectContaining({
          providerKey: "rss",
          rssFeedUrls: expect.arrayContaining(["https://github.blog/feed/"]),
        }),
      ]),
    });
    expect(result.value.drafts.map((draft) => draft.providerKey)).toEqual([
      "reddit",
      "hacker-news",
      "github-repo-radar",
      "rss",
    ...
    expect(result.value.drafts[0]?.sourceBindingDraft?.config).toMatchObject({
      subreddits: expect.arrayContaining(["LocalLLaMA", "MachineLearning"]),
    });
    expect(
      result.value.drafts[1]?.sourceBindingDraft?.config.scanPasses,
    ).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ query: "LLM evals", target: "story" }),
      ]),
    );
    expect(result.value.drafts[2]?.sourceBindingDraft?.config).toMatchObject({
      topics: expect.arrayContaining(["agents", "llm"]),
      languages: expect.arrayContaining(["Python", "TypeScript"]),
    });
    expect(result.value.drafts[3]?.sourceBindingDraft?.config.feedUrl).toBe(
      "https://github.blog/feed/",
    );
  });

  it("rejects unknown source packs", async () => {
    const interests = new FakeInterests();
    interests.add(makeInterest());
    ...
    const result = await new PlanInterestCoverageUseCase(
      ...
      ...scope,
      interestId: "interest-1",
      sourcePackKey: "not-a-pack",
    });

    expect(result.ok).toBe(false);
    expect(!result.ok && result.error.message).toBe(
      "Unknown source coverage pack",
    );
  });
  ...
  it("suggests a broad RSS search feed when no curated feed URLs are provided", async () => {
    ...
    expect(
      result.value.drafts[0]?.sourceBindingDraft?.config.feedUrl,
    ).toContain("https://news.google.com/rss/search?");
