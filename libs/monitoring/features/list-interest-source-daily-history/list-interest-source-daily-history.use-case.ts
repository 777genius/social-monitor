...
import type { ListInterestSourceDailyHistoryQuery } from './list-interest-source-daily-history.query';
import type {
  ListInterestSourceDailyHistoryResult,
} from './list-interest-source-daily-history.result';
import {
  buildInterestSourceDailyHistoryDayView,
  buildInterestSourceDailyHistorySummaryView,
} from './interest-source-daily-history-presenter';

type ListInterestSourceDailyHistoryFailure = DomainError;
...
export class ListInterestSourceDailyHistoryUseCase {
  constructor(
  ...
  async execute(
    query: ListInterestSourceDailyHistoryQuery,
  ): Promise<Result<ListInterestSourceDailyHistoryResult, ListInterestSourceDailyHistoryFailure>> {
    ...
    if (!Number.isInteger(query.days) || query.days < 1 || query.days > maxHistoryDays) {
      return err(new DomainError('validation.failed', `Interest source history days must be between 1 and ${maxHistoryDays}`));
    }
    ...
    const bindings = await this.sourceBindings.listByInterest({
      tenantId: query.tenantId,
      workspaceId: query.workspaceId,
      interestId: query.interestId,
    ...
    if (firstWindow === undefined || lastWindow === undefined) {
      return err(new DomainError('validation.failed', 'Interest source history days must be between 1 and 90'));
    }
    ...
    const days = windows.map((window) => buildInterestSourceDailyHistoryDayView({
      window,
    ...
    return ok({
      ...
      summary: buildInterestSourceDailyHistorySummaryView({
        days,
        jobs: scanJobs,
        schedulerDecisions,
        attempts,
...
const scanPoliciesBySourceBindingId = async (params: {
  ...
  readonly tenantId: ListInterestSourceDailyHistoryQuery['tenantId'];
  readonly workspaceId: ListInterestSourceDailyHistoryQuery['workspaceId'];
...
const normalizeProviderKeys = (
  ...
  if (providerKeys.length > 0 && normalized.length === 0) {
    return new DomainError('validation.failed', 'Interest source history providerKey filter must not be empty');
  }

  if (normalized.length > maxProviderFilters) {
    return new DomainError(
      'validation.failed',
      `Interest source history providerKey filter must include at most ${maxProviderFilters} providers`,
    );
  }
