...
export const buildStoryRankingTelemetrySnapshot = (
  ...
  const evidenceById = new Map(selection.selectedEvidence.map((item) => [item.feedItemId, item] as const));
  ...
  return {
    ...
    titleOnlyClusterShare: ratio(
      clusters.filter((cluster) => cluster.storyKey.startsWith('title:')).length,
      clusterCount,
    ),
    sameProviderDuplicateCount: sameProviderDuplicateCounts.reduce((total, count) => total + count, 0),
    ...
    clustersWithoutProviderMetrics: clusters.filter((cluster) =>
      clusterEvidence(cluster, evidenceById).every((item) => item.providerMetrics === undefined),
    ).length,
    topProviderKey: topProvider?.providerKey,
    topProviderClusterShare: ratio(topProvider?.clusterCount ?? 0, clusterCount),
  };
};

const clusterEvidence = (
  cluster: StoryCluster,
  evidenceById: ReadonlyMap<string, BriefingEvidenceItem>,
): readonly BriefingEvidenceItem[] =>
  [cluster.representativeFeedItemId, ...cluster.duplicateFeedItemIds]
    .flatMap((feedItemId) => evidenceById.get(feedItemId) ?? []);
...
const sameProviderDuplicateCount = (
  ...
  for (const item of clusterEvidence(cluster, evidenceById)) {
    providerCounts.set(item.providerKey, (providerCounts.get(item.providerKey) ?? 0) + 1);
  }
...
const topRepresentativeProvider = (
  ...
  evidenceById: ReadonlyMap<string, BriefingEvidenceItem>,
): { readonly providerKey: string; readonly clusterCount: number } | undefined => {
  const counts = new Map<string, number>();

  for (const cluster of clusters) {
    const providerKey = evidenceById.get(cluster.representativeFeedItemId)?.providerKey
