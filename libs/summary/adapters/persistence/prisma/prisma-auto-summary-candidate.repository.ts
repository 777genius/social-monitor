  ...
  async findDueCandidates(
    ...
    const rows = await this.prisma.$queryRaw<AutoSummaryCandidateRow[]>`
      with latest_summary as (
        ...
        group by tenant_id, workspace_id, topic_id
      )
      select
        p.tenant_id,
        p.workspace_id,
      ...
      group by p.tenant_id, p.workspace_id, p.topic_id, ls.latest_summary_requested_at
      having count(f.id) filter (
        where ls.latest_summary_requested_at is null
           or f.observed_at > ls.latest_summary_requested_at
      ) > 0
      order by max(f.observed_at) asc, p.topic_id asc
      limit ${params.limit}
    `;
