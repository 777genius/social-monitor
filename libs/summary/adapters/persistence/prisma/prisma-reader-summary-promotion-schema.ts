...
import { assertV3PromotionAttestation } from
  "./prisma-reader-summary-promotion-v3-schema";
import { booleans, dates, exactKeys, exactValue, invalid,
  nonEmptyUniqueStringArray, nonNegativeInteger, numbers, oneOf, requireArray,
  requireRecord, sameOrdered, stringArray, strings, units } from
...
export const assertExactPromotionAttestationPayload = (value: unknown): void => {
  ...
  for (const [index, raw] of items.entries()) {
    ...
    if (item.schemaVersion === "reader_post_promotion_attestation.v3") {
      assertV3PromotionAttestation(item, index);
      continue;
    }
