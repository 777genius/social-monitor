...
describe.each(cases)("offline adapter to final guard: $name", ({ inputs, mayMerge }) => {
  it.each(controls)("$name", async (control) => {
    const items = inputs.map((input, i) => ({
      ...
      publishedAt: new Date(String(input.publishedAt)), observedAt: new Date(String(input.observedAt)),
    }));
    ...
    const runTask = jest.fn(async (command: AgentRuntimeTaskCommand): Promise<AgentRuntimeTaskResult> => {
      ...
      if (control.name === "refusal") return { status: "failed", warnings: [], failure: {
      ...
      if (control.name === "no output") return { status: "completed" as const, warnings: [] };
