...
import { STORY_RANKING_POLICY_V1, type BriefingEvidenceSelection } from '../../domain';
...
describe('StoryRankingMetricsRecorder', () => {
  it('records production ranking and dedup gauges with the policy version label', () => {
    ...
    expect(metrics.latestGaugeValue('summary_story_ranking_average_signal', labels)).toBe(2.4);
    expect(metrics.latestGaugeValue('summary_story_ranking_cross_provider_cluster_share', labels)).toBe(0.5);
    expect(metrics.latestGaugeValue('summary_story_ranking_title_only_cluster_share', labels)).toBe(0.5);
    expect(metrics.latestGaugeValue('summary_story_ranking_same_provider_duplicates_total', labels)).toBe(1);
    expect(metrics.latestGaugeValue('summary_story_ranking_same_provider_duplicate_max', labels)).toBe(1);
    expect(metrics.latestGaugeValue('summary_story_ranking_clusters_without_provider_metrics', labels)).toBe(1);
    expect(metrics.latestGaugeValue('summary_story_ranking_top_provider_cluster_share', {
      ...labels,
...
const selection = (): BriefingEvidenceSelection => ({
