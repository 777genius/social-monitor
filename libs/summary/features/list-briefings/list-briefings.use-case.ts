...
export class ListBriefingsUseCase {
  ...
  async execute(query: ListBriefingsQuery): Promise<Result<ListBriefingsResult, ListBriefingsFailure>> {
    if (!Number.isInteger(query.limit) || query.limit < 1 || query.limit > MAX_LIMIT) {
      return err(new DomainError('validation.failed', 'Briefing page limit must be between 1 and 100', {
        limit: query.limit,
      }));
    }
    ...
    const items = await Promise.all(result.items.map(async (briefing) => {
