...
import { presentBriefingArtifact } from './briefing-artifact-presenter';

describe('presentBriefingArtifact', () => {
  it('preserves canonical URLs for UI citation links', () => {
    const artifact = BriefingArtifact.create({
      ...
      confidence: {
        level: 'medium',
        score: 0.7,
        rationale: 'Selected evidence is sufficient for a briefing.',
      },
      lineage: {
    ...
    expect(presentBriefingArtifact(artifact, {
      status: 'fresh',
