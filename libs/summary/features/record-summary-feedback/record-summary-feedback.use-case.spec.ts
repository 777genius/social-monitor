...
class FakeSummaryArtifacts implements SummaryArtifactRepositoryPort {
  ...
  async save(artifact: SummaryArtifact): Promise<void> {
    ...
    this.artifacts.set(
      `${snapshot.tenantId}:${snapshot.workspaceId}:${snapshot.summaryId}`,
      artifact,
    );
  }

  async list(
    query: ListSummaryArtifactsQuery,
  ): Promise<ListSummaryArtifactsResult> {
    return {
  ...
  async findById(
    ...
    return (
      this.artifacts.get(
        `${params.tenantId}:${params.workspaceId}:${params.summaryId}`,
      ) ?? null
    );
  }
}
...
class FakeSummaryFeedback implements SummaryFeedbackRepositoryPort {
  private readonly feedbackByIdempotencyKey = new Map<
    string,
    SummaryFeedback
  >();
  ...
  async findByIdempotencyKey(
    ...
    return (
      this.feedbackByIdempotencyKey.get(
  ...
  ): Promise<ListSummaryFeedbackResult> {
    return {
...
class CapturingSummaryMemory implements SummaryMemoryPort {
  readonly commands: Parameters<
    SummaryMemoryPort["recordSummaryFeedback"]
  >[0][] = [];

  async buildContext(): Promise<
    Awaited<ReturnType<SummaryMemoryPort["buildContext"]>>
  > {
    return {
      status: "disabled",
...
class FailingSummaryMemory extends CapturingSummaryMemory {
  override async recordSummaryFeedback(): Promise<
...
describe("RecordSummaryFeedbackUseCase", () => {
  it("records classified feedback with citation evidence and eval eligibility idempotently", async () => {
    ...
    await summaries.save(
      makeSummary({ tenantId: tenant, workspaceId: workspace }),
    );
  ...
  it("rejects feedback in strict memory failure mode before saving canonical feedback", async () => {
    const tenant = tenantId("tenant-1");
    const workspace = workspaceId("workspace-1");
    const summaries = new FakeSummaryArtifacts();
    const feedback = new FakeSummaryFeedback();
    ...
    expect(result).toEqual({
      ...
      error: expect.objectContaining({
        message: "memo-stack unavailable",
      }),
    });
    expect(feedback.all()).toHaveLength(0);
  });
