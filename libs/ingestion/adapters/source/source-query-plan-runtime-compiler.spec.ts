...
describe("DefaultSourceQueryPlanRuntimeCompiler", () => {
  ...
  it("keeps Reddit planned searches inside configured subreddit allowlist", () => {
    const compiler = new DefaultSourceQueryPlanRuntimeCompiler();
    ...
    const result = compiler.compile({
      ...
      runtimeConfig: {
        maxItems: 100,
        scanPasses: [
          { mode: "listing", subreddit: "ClaudeAI", listing: "top" },
          { mode: "listing", subreddit: "LocalLLaMA", listing: "top" },
          {
            mode: "search",
            query: "AI agents",
            allowedSubreddits: ["OpenAI"],
          },
        ],
      },
      plan: {
    ...
    expect(result.sourceQuery.parameters).toMatchObject({
      maxItems: 100,
      scanPasses: expect.arrayContaining([
        expect.objectContaining({
          mode: "search",
          allowedSubreddits: ["ClaudeAI", "LocalLLaMA", "OpenAI"],
        }),
        expect.objectContaining({
          mode: "listing",
          subreddit: "ClaudeAI",
