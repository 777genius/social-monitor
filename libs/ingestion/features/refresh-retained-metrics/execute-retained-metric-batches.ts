...
import { refreshBatches, sameTarget, targetProblem } from "./metric-refresh-admission";
import type {
  MetricRefreshManifest, MetricRefreshOutcome, PreservedMetricObservation, RefreshDigest,
  RetainedMetricFetchCapability, RetainedMetricInventory,
} from "./refresh-retained-metrics.contracts";

import type { MetricRefreshOperation } from "./metric-refresh-operation.contracts";

// Admission belongs to the two fixed use cases. The complete validated manifest
// digest is passed separately; execution never hashes a stripped manifest.
export type MetricExecutionManifest = Pick<MetricRefreshManifest, "operationId" | "evidencePath" | "scope" | "plannedAt" | "targets">;
type BatchEvidence = { observations: readonly PreservedMetricObservation[]; failure: string | null };
export class ExecuteRetainedMetricBatches {
  constructor(
  ...
  async execute(receipts: MetricRefreshOperation, manifest: MetricExecutionManifest, expectedSha: string): Promise<Result<readonly MetricRefreshOutcome[], string>> {
    receipts.assertHeld();
    ...
    for (const [index, targets] of refreshBatches(manifest.targets).entries()) {
      ...
      const reservation = { operationId: manifest.operationId, manifestDigest: expectedSha, targets: targets.map((t) => t.sourceItemId) };
