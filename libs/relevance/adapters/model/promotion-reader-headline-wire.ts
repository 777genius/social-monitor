export const promotionReaderHeadlineInstructions = [
  "Independently propose readerHeadline in this same batch for EVERY candidate; headline feasibility must not change content verdicts, scores, evidence or flags.",
  ...
  "evidenceAvailability is authoritative: when body_present, a collector title ending in ... alone does not mean source input is incomplete; use the complete bodyPreview when it supports a safe headline. If evidenceAvailability is truncated or a required source tail is unseen, return unavailable/incomplete_source. Never infer the tail. If confidence is below 0.8 or a claim cannot preserve all material qualifications, use a valid subject_label only where allowed below or unavailable; never fabricate support.",
  ...
  "Output allocation: reserve the original quality fields for EVERY candidate first. All readerHeadline objects together have a conservative budget of 2400 output tokens for the entire batch, at most floor(2400 / candidate count) per candidate including JSON overhead. Use compact JSON. If a complete faithful annotation cannot fit, emit {\"status\":\"unavailable\",\"reasonCode\":\"unresolved_qualifications\"}. Never shorten original quality evidence or omit a candidate to fund a headline; never return incomplete JSON.",
