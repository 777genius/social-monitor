...
type ApplicationNameRow = { readonly application_name: string };
...
describeWithPostgres('real Prisma query concurrency through the bounded pg Pool', () => {
  ...
  it('sets the real PostgreSQL session application_name from runtime identity', async () => {
    ...
    try {
      const rows = await lease.client.$queryRaw<ApplicationNameRow[]>`
