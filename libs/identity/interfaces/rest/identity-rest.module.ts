...
import { UsagePublicApiAuditWriterAdapter } from '../../adapters/usage/usage-public-api-audit-writer.adapter';
import { UsagePublicApiRateLimiterAdapter } from '../../adapters/usage/usage-public-api-rate-limiter.adapter';
...
@Module({
  ...
  providers: [
    ...
    },
    {
      ...
      useFactory: (
        workflow: CheckPublicApiRateLimitUseCase,
      ): PublicApiRateLimiterPort =>
        new UsagePublicApiRateLimiterAdapter(workflow),
      inject: [CheckPublicApiRateLimitUseCase],
    },
    {
      provide: IDENTITY_PUBLIC_API_AUDIT_WRITER,
      useFactory: (
        workflow: RecordPublicApiAuditEventUseCase,
      ): PublicApiAuditWriterPort =>
        new UsagePublicApiAuditWriterAdapter(workflow),
