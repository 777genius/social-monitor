...
export class ProjectSummaryReadyEventHandler {
  ...
  async handle(
  ...
  ): Promise<ProjectSummaryReadyEventResult> {
    ...
    return this.runtime.runIfAccepting(eventType, async () => {
      ...
      try {
        const result = await runWithTenantDatabaseAccess(parsed.payload, () =>
          this.projectSummaryReady.execute({ event: parsed }),
        );
...
const parseEvent = (
  ...
  if (
    payloadTenantId !== topLevelTenantId ||
    payloadWorkspaceId !== topLevelWorkspaceId
  ) {
    throw new DomainError(
      "validation.failed",
      "summary.ready payload scope must match event scope",
    );
  }
