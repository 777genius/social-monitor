...
export type SendDeliveryAttemptQueueCommand =
  QueueCommandEnvelope<SendDeliveryAttemptQueuePayload>;
...
export class SendDeliveryAttemptCommandHandler {
  ...
  ): Promise<SendDeliveryAttemptResult> {
    ...
    return this.runtime.runIfAccepting(command.commandType, async () => {
      ...
      try {
        const result = await runWithTenantDatabaseAccess(payload, () =>
          this.sendDeliveryAttempt.execute({
        ...
        this.recordMetric(
          isFailedDeliveryState(result.value.attempt.state)
            ? "failed"
            : "succeeded",
        );
        return result.value;
...
): SendDeliveryAttemptQueuePayload => ({
...
const readContent = (value: unknown): DeliveryContent => {
  if (typeof value !== "object" || value === null) {
    throw new Error(
      "Invalid send delivery attempt command payload field: content",
    );
  }
...
const readString = (
  ...
  if (typeof value !== "string" || value.trim().length === 0) {
    throw new Error(
      `Invalid send delivery attempt command payload field: ${field}`,
    );
  }
...
const isFailedDeliveryState = (
  state: SendDeliveryAttemptResult["attempt"]["state"],
): boolean =>
