...
export function resolveReaderValueRuntimeOptions(env: NodeJS.ProcessEnv): ReaderValueRuntimeOptions {
  const mode = env.READER_VALUE_MODE ?? 'legacy_v2';
  ...
  const loop = env.READER_VALUE_SCORING_LOOP ?? 'disabled';
  ...
  if (mode !== 'legacy_v2' && (scopes.length === 0 || backfill === undefined)) {
    throw new Error('Jev discovery requires explicit READER_VALUE_DISCOVERY_SCOPES and READER_VALUE_BACKFILL_FROM');
  }
  ...
  if (mode === 'jev_primary_v3') {
    ...
    if (poller.tenantId !== undefined && scopes.some((scope) => scope.tenantId !== poller.tenantId?.toLowerCase()
