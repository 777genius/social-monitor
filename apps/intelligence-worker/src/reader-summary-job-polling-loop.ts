  ...
  implements OnModuleInit, OnApplicationShutdown
{
  ...
  async onModuleInit(): Promise<void> {
    if (!this.options.enabled) {
      this.logger.info("readerSummary job polling loop disabled", {
    ...
    this.logger.info("readerSummary job polling loop started", {
  ...
  private async executeTick(trigger: "startup" | "interval"): Promise<void> {
    try {
      ...
      for (const job of jobs) {
        ...
        try {
          await this.handler.handle({
            ...
            schemaVersion: 1,
            correlationId: this.commandIds.next("readerSummary-job-poller", [
              trigger,
            ]),
            payload: {
      ...
      this.logger.info("readerSummary job polling loop tick completed", {
        trigger,
      ...
      this.logger.error("readerSummary job polling loop tick failed", {
        trigger,
        error: error instanceof Error ? error.message : String(error),
        worker: "intelligence-worker",
      });
