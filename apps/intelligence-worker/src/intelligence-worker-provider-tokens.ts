...
export type IntelligenceReaderSummaryJobLoopOptions = IntelligenceSummaryJobLoopOptions;
...
export type IntelligenceRabbitMqReaderSummaryQueueReaderOptions = IntelligenceRabbitMqSummaryQueueReaderOptions;
...
export type IntelligenceReaderSummaryQueueDrainLoopOptions = IntelligenceSummaryQueueDrainLoopOptions;
...
export const INTELLIGENCE_READER_SUMMARY_JOB_LOOP_OPTIONS = Symbol('INTELLIGENCE_READER_SUMMARY_JOB_LOOP_OPTIONS');
...
export const INTELLIGENCE_RABBITMQ_READER_SUMMARY_QUEUE_READER_OPTIONS =
  Symbol('INTELLIGENCE_RABBITMQ_READER_SUMMARY_QUEUE_READER_OPTIONS');
export const INTELLIGENCE_SUMMARY_QUEUE_DRAIN_LOOP_OPTIONS =
  Symbol('INTELLIGENCE_SUMMARY_QUEUE_DRAIN_LOOP_OPTIONS');
export const INTELLIGENCE_READER_SUMMARY_QUEUE_DRAIN_LOOP_OPTIONS =
...
export const resolveIntelligenceReaderSummaryJobLoopOptions = (
  ...
  if (loopMode !== 'enabled' && loopMode !== 'disabled') {
    throw new Error('INTELLIGENCE_READER_SUMMARY_JOB_LOOP must be "enabled" or "disabled"');
  }

  const tenant = emptyToUndefined(env.INTELLIGENCE_READER_SUMMARY_JOB_LOOP_TENANT_ID);
  const workspace = emptyToUndefined(env.INTELLIGENCE_READER_SUMMARY_JOB_LOOP_WORKSPACE_ID);
  ...
  return {
    enabled: loopMode === 'enabled',
    intervalMs: parseBoundedInteger(env.INTELLIGENCE_READER_SUMMARY_JOB_LOOP_INTERVAL_MS, 60_000, 1_000, 3_600_000),
    limit: parseBoundedInteger(env.INTELLIGENCE_READER_SUMMARY_JOB_LOOP_LIMIT, 20, 1, 100),
    runOnStart: parseBoolean(env.INTELLIGENCE_READER_SUMMARY_JOB_LOOP_RUN_ON_START, true),
    tenantId: tenant,
    workspaceId: workspace,
  };
};
...
export const resolveIntelligenceRabbitMqReaderSummaryQueueReaderOptions = (
  ...
  queue: nonEmptyOrFallback(env.RABBITMQ_READER_SUMMARY_QUEUE, 'jobs.reader_summary.execute'),
  deadLetterExchange: parseRabbitMqDeadLetterExchange(env.RABBITMQ_DEAD_LETTER_EXCHANGE, {
    runtimeProfile: env.SOCIAL_MONITOR_RUNTIME_PROFILE,
    settingName: 'INTELLIGENCE_SUMMARY_QUEUE_READER=rabbitmq',
  }),
...
export const resolveIntelligenceReaderSummaryQueueDrainLoopOptions = (
  ...
  const defaultMode = env.INTELLIGENCE_SUMMARY_QUEUE_READER === 'rabbitmq' ? 'enabled' : 'disabled';
  const loopMode = env.INTELLIGENCE_READER_SUMMARY_QUEUE_DRAIN_LOOP ?? (env.NODE_ENV === 'test' ? 'disabled' : defaultMode);

  if (loopMode !== 'enabled' && loopMode !== 'disabled') {
    throw new Error('INTELLIGENCE_READER_SUMMARY_QUEUE_DRAIN_LOOP must be "enabled" or "disabled"');
  }

  return {
    enabled: loopMode === 'enabled',
    intervalMs: parseBoundedInteger(env.INTELLIGENCE_READER_SUMMARY_QUEUE_DRAIN_INTERVAL_MS, 5_000, 500, 3_600_000),
    limit: parseBoundedInteger(env.INTELLIGENCE_READER_SUMMARY_QUEUE_DRAIN_LIMIT, 20, 1, 100),
    runOnStart: parseBoolean(env.INTELLIGENCE_READER_SUMMARY_QUEUE_DRAIN_RUN_ON_START, true),
  };
};
