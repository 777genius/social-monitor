...
import {
  ...
  type IngestionRabbitMqScanQueueReaderOptions,
} from './ingestion-worker-provider-tokens';

export const INGESTION_SCAN_COMMAND_QUEUE_READER = Symbol('INGESTION_SCAN_COMMAND_QUEUE_READER');
...
export interface ScanCommandQueueReaderPort {
  drain(params: {
...
@Injectable()
export class InMemoryScanCommandQueueReader implements ScanCommandQueueReaderPort {
...
@Injectable()
export class RabbitMqScanCommandQueueReader implements ScanCommandQueueReaderPort {
  private routeAsserted = false;

  constructor(
    private readonly channel: RabbitMqScanQueueReaderChannelPort,
    @Inject(INGESTION_RABBITMQ_SCAN_QUEUE_READER_OPTIONS)
    private readonly options: IngestionRabbitMqScanQueueReaderOptions,
  ) {}
  ...
  async drain(params: {
    ...
    const deliveries: QueueCommandDelivery[] = [];
...
const parseQueueCommand = (
  ...
  if (typeof parsed.commandId !== 'string' || parsed.commandId.trim().length === 0) {
    throw new Error('Invalid RabbitMQ command envelope: commandId');
  }

  if (typeof parsed.commandType !== 'string' || parsed.commandType.trim().length === 0) {
    throw new Error('Invalid RabbitMQ command envelope: commandType');
  }

  if (typeof parsed.correlationId !== 'string' || parsed.correlationId.trim().length === 0) {
    throw new Error('Invalid RabbitMQ command envelope: correlationId');
  }

  if (typeof parsed.schemaVersion !== 'number' || !Number.isInteger(parsed.schemaVersion)) {
    throw new Error('Invalid RabbitMQ command envelope: schemaVersion');
  }

  if (typeof payload !== 'object' || payload === null || Array.isArray(payload)) {
    throw new Error('Invalid RabbitMQ command envelope: payload');
  }
