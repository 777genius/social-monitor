...
bool verifyReaderDisplayHeadline({
  ...
  final isV3 =
      payload['schemaVersion'] == 'reader_post_promotion_attestation.v3';
  final presentation = payload['presentation'];
  final seal = isV3 && presentation is Map<String, Object?>
      ? presentation['displayHeadline']
      : payload['displayHeadline'];
  if (seal == null &&
      !payload.containsKey('displayHeadline') &&
      !isV3 &&
      headline == null &&
      source == null &&
      outerSeal == null) {
  ...
  if (headline['status'] == 'unavailable') {
    // Compatibility headlines are a legacy V2 representation. A V3
    // attestation is display-ready only when its sealed presentation is an
    // accepted, source-bound headline; unknown/missing V3 states fail closed.
  ...
  if (!_safeUtf16(sourceTitle) ||
      !_safeUtf16(body) ||
      sourceTitle.length > 2000 ||
      body.length > (isV3 ? 64000 : 12000)) {
    return false;
  }
  ...
  if (binding is! Map<String, Object?> ||
      ...
      whole['titleLength'] != sourceTitle.length ||
      whole['bodyLength'] != body.length ||
      confidence is! num ||
      !confidence.isFinite ||
      confidence < .8 ||
      confidence > 1) {
  ...
  if (!_digestPattern(binding['reviewedInputDigest']) ||
      (isV3 && !_digestPattern(binding['interestDigest']))) {
    return false;
  }
  if (isV3) {
    // Interest plaintext is deliberately unavailable in the V3 public
    // contract. Its digest is verified by the locked-assessment publisher.
    return _validReferencesAndQualifications(
      headline,
      sourceTitle,
      body,
      title: title,
...
bool _validReferencesAndQualifications(
  Map<String, Object?> headline,
  String sourceTitle,
  String body, {
  required String title,
  required Map<String, Object?> whole,
  bool isV3 = false,
}) {
  final support = headline['support'];
  final qualifications = headline['qualifications'];
  if (!_references(support, sourceTitle, body, isV3: isV3) ||
      qualifications is! List<Object?> ||
  ...
  for (final q in qualifications) {
    ...
    if (!_concise(phrase) ||
        ...
        !phrases.add(phrase) ||
        !_references(q['evidence'], sourceTitle, body, isV3: isV3)) {
      return false;
    }
    collect(q['evidence']);
  }
  ...
  if (references.length > 8 ||
      ...
      referenceQuotes.fold<int>(
            0,
            (sum, quote) => sum + jsonEncode(quote).length,
          ) >
          1024) {
    return false;
  }
  if (headline['kind'] == 'claim') {
    return whole['qualificationJudgment'] ==
        (phrases.isEmpty ? 'none' : 'preserved');
  }
  final refs = (support! as List<Object?>).cast<Map<String, Object?>>();
  final quotes = refs.map((ref) => ref['quote']! as String).toList();
  return whole['qualificationJudgment'] == 'subject_only' &&
      phrases.isEmpty &&
      quotes.length >= 2 &&
      quotes.length <= 3 &&
      refs.every((ref) => _wholeSubjectToken(ref, sourceTitle, body)) &&
      RegExp(
        r'^\p{Lu}[\p{L}\p{M}\p{N}-]{1,39}$',
        unicode: true,
      ).hasMatch(quotes.first) &&
      {
        'benchmark',
        'compiler',
        'model',
        ...
        'safety',
        'latency',
      }.contains(quotes.last) &&
      (quotes.length != 3 ||
          RegExp(r'^v\d{1,3}(?:\.\d{1,3}){0,2}$').hasMatch(quotes[1])) &&
...
bool _references(
  ...
  for (final ref in value) {
    if (ref is! Map<String, Object?> ||
        !_keys(ref, {'field', 'start', 'end', 'quote'}) ||
...
bool _digestPattern(Object? value) =>
...
bool _wholeSubjectToken(Map<String, Object?> ref, String title, String body) {
  ...
  return (before.isEmpty ||
          !continuation.hasMatch(String.fromCharCode(before.last))) &&
      (after.isEmpty ||
          !continuation.hasMatch(String.fromCharCode(after.first)));
}

bool _faithfulSourceTitle(String title, Map<String, Object?> source) {
  final sourceTitle = (source['title']! as String).trim().replaceFirst(
    RegExp(r'^X post by @[^:]+:\s*', caseSensitive: false),
    '',
  );
...
bool _concise(String text) =>
    text.isNotEmpty &&
    text.length <= 119 &&
    text == text.trim() &&
    _safeUtf16(text) &&
    !RegExp(
      r'[\x00-\x1f\x7f-\x9f\u2028\u2029\u202a-\u202e\u2066-\u2069<>]|https?://|\.\.\.|…',
    ).hasMatch(text);
