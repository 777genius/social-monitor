# Settings Feature Agent Rules

This feature is the `settings` bounded context.
Read this file before changing anything under `apps/frontend/features/settings`.
...
docs/context_map.md
lib/social_monitor_settings.dart
lib/src/presentation/routes/settings_feature_route.dart
lib/src/presentation/composition/settings_feature_module.dart
lib/src/presentation/composition/settings_feature_module_host.dart
lib/src/presentation/pages/settings_feature_page.dart
```

## Bounded Context Purpose

Owns workspace preferences, notification settings, account controls and diagnostics language.

## Growth Triggers

- preferences need validation, policy or use cases;
- workspace settings become entities or value objects;
- settings persistence or generated API mapping appears;
- notification/digest configuration needs commands or queries;
- a store starts coordinating multiple settings workflows;
- another feature needs a settings contract or preference read model.
