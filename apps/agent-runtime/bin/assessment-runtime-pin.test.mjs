...
test("sm.2 and its exact launcher are admitted; old pin and altered bytes fail closed", async () => {
  ...
  try {
    ...
    execFileSync("tar", ["-xzf", resolve("vendor/vioxen-subscription-runtime-0.1.0-main.42-sm.2.tgz"),
    ...
    assert.equal((await inspector.inspect(launcher)).runtimePackageVersion, "0.1.0-main.42-sm.2");
    const manifestPath = join(modules, "package.json");
    const manifest = await readFile(manifestPath, "utf8");
    await writeFile(manifestPath, manifest.replace("0.1.0-main.42-sm.2", "0.1.0-main.42-sm.1"));
