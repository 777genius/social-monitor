...
describe("reader summary daily terminal-set receipt", () => {
  it("emits only the exact six-terminal canonical receipt", () => {
    ...
    expect(receipt).toEqual({
      ...
      terminalCount: 6,
      terminalSetSha256: canonicalInvalidProductRetrySetSha256(
  ...
  it.each([
    ...
    [
      "extra",
      (value: ReaderSummaryDailyTerminalSetRow[]) => [...value, value[0]!],
    ],
    [
      "wrong date",
      (value: ReaderSummaryDailyTerminalSetRow[]) =>
        value.map((row, index) =>
          index === 0 ? { ...row, requestedUtcDate: "2026-07-24" } : row,
        ),
    ],
    [
      "wrong state",
      (value: ReaderSummaryDailyTerminalSetRow[]) =>
        value.map((row, index) =>
          index === 0 ? { ...row, outcome: "FINALIZED" } : row,
        ),
    ],
    [
      "wrong reason",
      (value: ReaderSummaryDailyTerminalSetRow[]) =>
        value.map((row, index) =>
          index === 0 ? { ...row, reasonCode: "other" } : row,
        ),
    ],
    [
      "zero signals",
    ...
    [
      ...
      (value: ReaderSummaryDailyTerminalSetRow[]) =>
        value.map((row, index) =>
          index === 0 ? { ...row, signalCount: "01" } : row,
        ),
    ],
    [
      "wrong attempt",
      (value: ReaderSummaryDailyTerminalSetRow[]) =>
        value.map((row, index) =>
          index === 0 ? { ...row, attemptOrdinal: "02" } : row,
        ),
    ],
    [
      "wrong model hash",
      (value: ReaderSummaryDailyTerminalSetRow[]) =>
        value.map((row, index) =>
          index === 0 ? { ...row, modelJobIdentity: "A".repeat(64) } : row,
        ),
    ],
    [
      ...
      (value: ReaderSummaryDailyTerminalSetRow[]) =>
        value.map((row, index) =>
          index === 0 ? { ...row, sourceAuthoritySha256: "f".repeat(63) } : row,
        ),
    ],
  ])("fails closed for %s authority", (_label, mutate) => {
    expect(() =>
      buildReaderSummaryDailyTerminalSetReceipt(mutate(rows())),
    ).toThrow(/terminal/i);
  });
  ...
  it("keeps the reviewed terminal-set digest parity vector unchanged", () => {
    expect(
      buildReaderSummaryDailyTerminalSetReceipt(rows()).terminalSetSha256,
    ).toBe("228d6997c74f88bfa0fdcb745c81cc4e05ad1bc8fecb83796ad34f1de6c6de79");
  });
  ...
  it.each([
    ...
    [
      "wrong reason value",
      (line: string) =>
        line.replace("invalid-product-retry-set-v1", "other-retry-set"),
    ],
    [
      "wrong attempt value",
      (line: string) => line.replace('"terminalCount":6', '"terminalCount":7'),
    ],
    [
      "wrong hash",
      (line: string) => line.replace(/[0-9a-f]{64}(?="\}\n$)/u, "A".repeat(64)),
    ],
    ...
    expect(() =>
      parseReaderSummaryDailyTerminalSetReceiptLine(mutate(line)),
    ).toThrow(/receipt/i);
  });
  ...
  it("accepts exactly one compact canonical seven-key JSON line", () => {
    ...
    expect(parseReaderSummaryDailyTerminalSetReceiptLine(line)).toEqual(
      receipt,
    );
  });
});
