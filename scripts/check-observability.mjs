...
if (collectorBlock.includes("\n    ports:")) {
  violations.push(
    `${composePath}: collector telemetry endpoints must not be published on host ports`,
  );
}

for (const file of [...globSync("apps/**/*.ts"), ...globSync("libs/**/*.ts")]) {
  if (
...
for (const { file, document } of readJsonFiles(alertsDir)) {
  ...
  for (const alert of document.alerts ?? []) {
    ...
    if (!dashboardPanels.has(panelKey)) {
      violations.push(
        `${file}: alert "${alert.alertId}" references missing dashboard panel "${panelKey}"`,
      );
    }
  }
}
...
function requireArray(file, value, field) {
  if (!Array.isArray(value) || value.length === 0) {
    violations.push(
      `${file}: missing required non-empty array field "${field}"`,
    );
  }
}
...
function requireFileFragments(path, fragments) {
  if (!existsSync(path)) {
    violations.push(
      `${relativePath(path)}: required observability file is missing`,
    );
    return;
  }
  ...
  for (const fragment of fragments) {
    if (!source.includes(fragment)) {
      violations.push(
        `${relativePath(path)}: missing required fragment "${fragment}"`,
      );
    }
  }
}
