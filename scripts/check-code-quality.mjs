...
for (const useCaseFile of globSync("libs/**/features/**/*.use-case.ts")) {
  ...
  if (!existsSync(specFile)) {
    addViolation(
      useCaseFile,
      `feature use case must have focused unit spec "${relative(process.cwd(), specFile)}"`,
    );
  } else {
    const specSource = readFileSync(specFile, "utf8");
    const useCaseClass = readFileSync(useCaseFile, "utf8").match(
      /export\s+class\s+([A-Za-z0-9_]+UseCase)\b/,
    )?.[1];

    if (!useCaseClass) {
      addViolation(
        useCaseFile,
        "feature use case file must export a *UseCase class",
      );
    } else if (!specSource.includes(useCaseClass)) {
      addViolation(
        specFile,
        `feature use case spec must reference ${useCaseClass}`,
      );
    }
    ...
    if (!/\b(it|test)\s*\(/.test(specSource)) {
      addViolation(
        specFile,
        "feature use case spec must include at least one executable test",
      );
    }
  }
  ...
  if (/throw\s+new\s+DomainError\s*\(/.test(source)) {
    addViolation(
      useCaseFile,
      "feature use case must return err(new DomainError(...)) instead of throwing DomainError",
    );
  }
  ...
  if (/throw\s+new\s+Error\s*\(/.test(source)) {
    addViolation(
      useCaseFile,
      "feature use case must return Result failures instead of throwing generic Error",
    );
  }
  ...
  if (
    ...
    addViolation(
      useCaseFile,
      "feature use case must use injected Clock instead of wall-clock time",
    );
  }
  ...
  if (/\bnew\s+InMemory[A-Za-z0-9_]*\s*\(/.test(source)) {
    addViolation(
      useCaseFile,
      "feature use case must depend on ports, not concrete in-memory adapters",
    );
  }
}
...
for (const controllerFile of globSync(
  ...
  if (
    /\bNumber\s*\(\s*limitQuery\s*\)/.test(source) ||
    /\bconst\s+parseLimit\s*=/.test(source)
  ) {
    addViolation(
      controllerFile,
      "REST controllers must use parsePaginationLimit helpers for page limits, not ad hoc Number(limitQuery) parsing",
    );
  }
  ...
  if (/\.then\s*\(/.test(source)) {
    addViolation(
      controllerFile,
      "REST controllers must use async/await for transport flow instead of promise chains",
    );
  }
}
...
for (const file of [
  ...
  if (/\bconsole\.(log|info|warn|error|debug)\s*\(/.test(source)) {
    addViolation(
      file,
      "production code must use structured logging ports/adapters, not console.*",
    );
  }
  ...
  if (!normalized.includes("/generated/") && lineCount > maxLines) {
    addViolation(
      file,
      `production file exceeds ${maxLines} line budget (${lineCount}); split by responsibility before adding features`,
    );
  }

  if (
    /\brandomUUID\s*\(/.test(source) &&
    !directIdentityGenerationAllowedFiles.has(normalized)
  ) {
    addViolation(
      file,
      "production code must use IdGenerator or RequestCorrelationIdFactory instead of direct randomUUID",
    );
  }

  if (
    /\b(sensitiveKeyPattern|secretKeyPattern|bearerPattern|basicPattern|generatedSecretPattern|urlWithPasswordPattern)\b/.test(
      source,
    ) &&
    ...
    addViolation(
      file,
      "sensitive redaction patterns must live in shared-kernel redaction helpers, not be duplicated",
    );
  }

  if (
    /\b(blockedHosts|isPrivateIp|isPrivateOrLocalNetworkHost|localhost\.localdomain|169\s*&&\s*second\s*===\s*254)\b/.test(
      source,
    ) &&
    ...
    addViolation(
      file,
      "outbound URL SSRF policy must live in shared-kernel outbound URL helpers, not be duplicated",
    );
  }
}
...
for (const file of productionTsFiles("libs/**/interfaces/**/*.ts")) {
  ...
  if (source.includes("process.env")) {
    addViolation(
      file,
      "interface transport/support files must receive env-derived config through provider tokens, not read process.env",
    );
  }
}
...
for (const file of productionTsFiles("apps/**/src/*.controller.ts")) {
  ...
  if (source.includes("process.env")) {
    addViolation(
      file,
      "app controllers must receive env-derived readiness/config through providers, not read process.env",
    );
  }
  ...
  if (
    ...
    addViolation(
      file,
      "app controllers must receive Clock-backed timestamps through providers, not read wall-clock time",
    );
  }
  ...
  if (/\bprocess\.uptime\s*\(/.test(source)) {
    addViolation(
      file,
      "app controllers must receive uptime through providers, not read process uptime directly",
    );
  }
}
...
for (const file of [
  ...
  if (source.includes("process.env")) {
    addViolation(
      file,
      "adapters must receive runtime configuration explicitly from composition roots, not read process.env",
    );
  }
  ...
  if (
    ...
    addViolation(
      file,
      "adapters must receive Clock explicitly instead of reading wall-clock time",
    );
  }
  ...
  if (/\brandomUUID\s*\(/.test(source)) {
    addViolation(
      file,
      "adapters must receive IdGenerator explicitly instead of generating identities directly",
    );
  }
  ...
  if (/\bnew\s+(SystemClock|CryptoIdGenerator)\s*\(/.test(source)) {
    addViolation(
      file,
      "adapters must receive Clock/IdGenerator instances explicitly from composition roots, not create defaults",
    );
  }
  ...
  if (
    ...
    addViolation(
      file,
      "HTTP adapters must use AbortSignal.timeout for outbound fetch calls",
    );
  }
}
...
for (const file of [
  ...
  if (
    ...
    addViolation(
      file,
      "RabbitMQ queue declarations must use rabbitMqDurableQueueArguments for DLX/quorum arguments",
    );
  }
}
...
for (const file of productionTsFiles("libs/**/adapters/**/prisma/**/*.ts")) {
  ...
  if (!/from\s+['"]@social-monitor\/platform-persistence['"]/.test(source)) {
    addViolation(
      file,
      "Prisma persistence writes must use withPrismaWriteRetry from @social-monitor/platform-persistence",
    );
  }
  ...
  if (
    ...
    addViolation(
      file,
      "Prisma write transactions must set Serializable isolation and rely on P2034 retry",
    );
  }
}
...
for (const file of productionTsFiles("apps/**/src/*loop.ts")) {
  ...
  if (
    ...
    addViolation(
      file,
      "worker loops must use injected command id/clock helpers instead of wall-clock time",
    );
  }
}
...
for (const file of globSync("scripts/check-*.mjs")) {
  ...
  const source = readFileSync(file, "utf8");
  if (
    source.includes("provenanceRequirements") &&
    !source.includes(evidenceProvenanceHelperImport)
  ) {
    addViolation(
      file,
      "evidence provenance validators must use scripts/lib/evidence-provenance.mjs instead of local copies",
    );
  }
  if (
    ...
    addViolation(
      file,
      "evidence artifact provenance validators must also use validateRealEvidenceIdentityStrings for real artifact identity fields",
    );
  }
  for (const pattern of forbiddenLocalEvidenceProvenancePatterns) {
    if (pattern.test(source)) {
      addViolation(
        file,
        "evidence provenance constants and real-string checks must live in scripts/lib/evidence-provenance.mjs",
      );
    }
  }
}
...
for (const docFile of globSync("docs/**/*.md")) {
  ...
  if (
    /^-\s+(?:Pending commit\b|Introduced by\b|PR\s+\d+.+\b(?:added|strengthened)\s+in\s+(?:current\s+branch|this\s+(?:quality|implementation)\s+pass)\b)/m.test(
      source,
    )
  ) {
    addViolation(
      docFile,
      "committed evidence docs must not contain temporary commit evidence markers",
    );
  }
}
