...
if (contract.defaultLegalHoldBehavior !== "skip_purge_and_record_exception") {
  violations.push(
    `${contractPath}: defaultLegalHoldBehavior must be skip_purge_and_record_exception`,
  );
}
...
for (const policy of tablePolicies) {
  if (policyByTable.has(policy.table)) {
    violations.push(
      `${contractPath}: duplicate retention policy for table "${policy.table}"`,
    );
  }
...
for (const table of mappedTables) {
  if (!policyByTable.has(table)) {
    violations.push(
      `${contractPath}: missing retention policy for Prisma table "${table}"`,
    );
  }
}
...
for (const policy of tablePolicies) {
  if (!mappedTableSet.has(policy.table)) {
    violations.push(
      `${contractPath}: retention policy references unknown Prisma table "${policy.table}"`,
    );
  }
  ...
  for (const field of [
    ...
    if (
      ...
      policy[field].trim().length === 0
    ) {
      violations.push(
        `${contractPath}: policy for "${policy.table}" must define non-empty ${field}`,
      );
    }
  }
  ...
  if (
    ...
    violations.push(
      `${contractPath}: policy for "${policy.table}" retentionDays must be 0..3650`,
    );
  }

  if (typeof policy.exportable !== "boolean") {
    violations.push(
      `${contractPath}: policy for "${policy.table}" exportable must be boolean`,
    );
  }

  if (typeof policy.legalHoldAware !== "boolean") {
    violations.push(
      `${contractPath}: policy for "${policy.table}" legalHoldAware must be boolean`,
    );
  }

  if (
    policy.retentionDays === 0 &&
    !String(policy.deleteMode).startsWith("retain_until_")
  ) {
    violations.push(
      `${contractPath}: policy for "${policy.table}" with retentionDays 0 must use retain_until_* deleteMode`,
    );
  }
}
...
for (const requiredTable of [
  ...
  if (!policyByTable.has(requiredTable)) {
    violations.push(
      `${contractPath}: missing critical retention table "${requiredTable}"`,
    );
  }
}

for (const exportableTable of [
  "users",
  "memberships",
  "interests",
  "source_bindings",
  ...
  if (policyByTable.get(exportableTable)?.exportable !== true) {
    violations.push(
      `${contractPath}: "${exportableTable}" must be exportable for MVP DSAR/export coverage`,
    );
  }
}
...
for (const operationalTable of [
  ...
  if (policy?.exportable !== false) {
    violations.push(
      `${contractPath}: operational table "${operationalTable}" must not be user-exportable`,
    );
  }
  if (policy?.retentionDays > 365) {
    violations.push(
      `${contractPath}: operational table "${operationalTable}" retentionDays must be <= 365`,
    );
  }
}
...
if (!runbookPath || !existsSync(runbookPath)) {
  ...
  !readFileSync(runbookPath, "utf8")
    .toLowerCase()
    .includes((runbookAnchor ?? "").replaceAll("-", " "))
) {
  violations.push(
    `${contractPath}: runbook anchor must point to an existing runbook section`,
  );
}
