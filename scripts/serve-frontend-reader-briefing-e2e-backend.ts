...
const tenantId = 'tenant-frontend-reader-briefing-e2e';
const workspaceId = 'workspace-frontend-reader-briefing-e2e';
const userId = 'user-frontend-reader-briefing-e2e';
const briefingId = 'briefing-frontend-reader-briefing-e2e';
const summaryId = 'summary-frontend-reader-briefing-e2e';
...
const briefingArtifact =
  loadFrontendFixtureBriefingArtifact() ?? defaultBriefingArtifact;
const servedBriefingId =
  typeof briefingArtifact.briefingId === 'string'
    ? briefingArtifact.briefingId
    : briefingId;

function loadFrontendFixtureBriefingArtifact(): JsonObject | null {
  const fixturePath = process.env.FRONTEND_READER_BRIEFING_FIXTURE_PATH?.trim();
  if (fixturePath === undefined || fixturePath.length === 0) {
    return null;
  }
  if (!existsSync(fixturePath)) {
    throw new Error(
      `FRONTEND_READER_BRIEFING_FIXTURE_PATH does not exist: ${fixturePath}`,
    );
  }

  const parsed = JSON.parse(readFileSync(fixturePath, 'utf8')) as unknown;
  if (!isJsonObject(parsed) || !isJsonObject(parsed.briefingArtifact)) {
    throw new Error(
      'FRONTEND_READER_BRIEFING_FIXTURE_PATH must contain briefingArtifact object',
    );
  }
  if (typeof parsed.briefingArtifact.briefingId !== 'string') {
    throw new Error('briefingArtifact.briefingId must be a string');
  }

  return parsed.briefingArtifact;
}
...
async function handleRequest(
  ...
  if (request.method === 'GET' && url.pathname === '/briefings') {
    sendJson(response, 200, { items: [briefingArtifact], nextCursor: null });
    return;
  }

  if (
    request.method === 'GET' &&
    url.pathname === `/briefings/${servedBriefingId}`
  ) {
    sendJson(response, 200, briefingArtifact);
    return;
  }
  ...
  if (
    ...
    sendJson(response, 201, {
      ...
      profile: {
        id: 'profile-frontend-reader-briefing-e2e',
        userId,
        topicWeights: [],
        sourceWeights: [],
        keywordWeights: [],
        blockedProviderKeys: [],
        mutedKeywords: [],
        rulesVersion: 'reader-briefing-e2e.v1',
        updatedAt: checkedAt,
      },
    });
    return;
...
const requestedPort = Number.parseInt(
  process.env.FRONTEND_READER_BRIEFING_E2E_PORT ?? '0',
  10,
);
const listenPort =
