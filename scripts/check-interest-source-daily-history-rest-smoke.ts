...
const createInterest = async (params: {
  ...
  const topic = await request(params.httpServer)
    .post('/interests')
    .set(params.headers)
    .set('idempotency-key', 'interest-source-daily-history')
    .send({
...
const seedCompletedScan = async (params: {
  ...
  const enqueued = ScanJob.request({
    ...
    idempotencyKey: `interest-source-daily-history-${params.scanJobId}`,
    requestedAt,
    ...
    records: [
      {
        ...
        causationId: `interest-source-daily-history-${params.decisionKey}`,
      },
    ],
  });
};
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const tenant = tenantId('tenant-interest-source-daily-history-rest-smoke');
    const workspace = workspaceId('workspace-interest-source-daily-history-rest-smoke');
    const adminHeaders = {
      'x-tenant-id': tenant,
    ...
    const otherWorkspaceHeaders = {
      'x-tenant-id': tenant,
      'x-workspace-id': workspaceId('workspace-interest-source-daily-history-rest-smoke-other'),
      'x-workspace-role': 'viewer',
    };
    const otherTenantHeaders = {
      'x-tenant-id': tenantId('tenant-interest-source-daily-history-rest-smoke-other'),
      'x-workspace-id': workspace,
      'x-workspace-role': 'viewer',
    };

    const interestId = await createInterest({
    ...
    await seedCompletedScan({
      ...
      scanJobId: 'interest-source-daily-history-fake-source-success',
      completedAt,
    });
    await seedCompletedScan({
      scanJobs,
      ...
      scanJobId: 'interest-source-daily-history-rss-rate-limit',
    ...
    assert(history.body.interestId === interestId, 'interest source daily history must preserve interest id');
    assert(history.body.days.length === 1, 'interest source daily history must honor days query');
    assert(history.body.summary.sourceBindingCount === 3, 'interest source daily history must count source bindings');
    assert(history.body.summary.configuredSourceBindingCount === 3, 'interest source daily history must count configured bindings');
    assert(history.body.summary.scannedSourceBindingCount === 2, 'interest source daily history must count scanned bindings');
    assert(history.body.summary.unscannedSourceBindingCount === 1, 'interest source daily history must count unscanned bindings');
    assert(history.body.summary.scanCoverageState === 'partial', 'interest source daily history must expose partial scan coverage');
    assert(
      history.body.summary.signals.includes('partial_scan_coverage'),
      'interest source daily history must signal partial scan coverage',
    );
    assert(
      history.body.summary.signals.includes('unscanned_source_bindings'),
      'interest source daily history must signal unscanned source bindings',
    );
    assert(history.body.summary.succeededScans === 1, 'interest source daily history must count successful scans');
    assert(history.body.summary.failedScans === 1, 'interest source daily history must count failed scans');
    assert(history.body.summary.rateLimitedScans === 1, 'interest source daily history must classify rate limits');
    assert(history.body.summary.schedulerDecisionCount === 2, 'interest source daily history must count scheduler decisions');
    assert(history.body.summary.schedulerEnqueuedCount === 1, 'interest source daily history must count scheduler enqueues');
    assert(history.body.summary.schedulerSkippedCount === 1, 'interest source daily history must count scheduler skips');
    assert(
      history.body.summary.schedulerSkippedByReason.rateLimitBackoff === 1,
      'interest source daily history must expose rate-limit scheduler skip breakdown',
    );
    assert(
      ...
      'interest source daily history must expose successful fake-source provider breakdown',
    );
    assert(
      history.body.summary.providerBreakdown.some(
        (provider: {
      ...
      'interest source daily history must expose unscanned Hacker News provider breakdown',
    );
    assert(
      history.body.summary.providerBreakdown.some(
        (provider: {
      ...
      'interest source daily history must expose RSS rate-limit provider breakdown',
    );
    assert(
      history.body.days[0].providerBreakdown.length === 3,
      'interest source daily history day view must expose per-provider breakdown',
    );
    ...
    console.log('Interest source daily history REST smoke OK');
  } finally {
    await app.close();
  }
}
