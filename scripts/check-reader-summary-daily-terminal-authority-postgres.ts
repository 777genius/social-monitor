...
const readOnlyTables = ["reader_summary_publications", "reader_summary_publication_slots", "reader_summary_weekly_publication_evidence"] as const, directAccessDeniedTables = ["reader_summary_jobs", "reader_summary_production_recovery_leases", "reader_summary_production_recovery_days", "reader_summary_production_recovery_dry_runs", "reader_summary_recovery_receipts", "reader_summary_weekly_certification_seals"] as const, dailyTerminalMigration = "20260730120000_reader_summary_daily_terminal_authority", terminalCapabilityMigration = "20260801120000_reader_summary_daily_terminal_runtime_capability", telemetryMigration = "20260824120000_reader_summary_daily_model_job_telemetry", defaultAclMigration = "20260824121000_reader_summary_daily_function_global_default_acl", terminalRole = "social_monitor_reader_summary_daily_terminal";
    ...
    WITH terminal_functions AS (
      ...
      EXISTS (SELECT 1 FROM pg_proc owned_proc JOIN pg_roles owner ON owner.oid = owned_proc.proowner
        WHERE owner.rolname = 'social_monitor_reader_summary_publication_owner'
          ...
          AND owned_proc.oid NOT IN ('claim_reader_summary_daily_canonical_recovery_v4(uuid,uuid,text,timestamptz)'::regprocedure, 'renew_reader_summary_daily_canonical_recovery_v4_lease(uuid,uuid,date,character,smallint,text,bigint,timestamptz)'::regprocedure, 'mark_reader_summary_daily_canonical_recovery_v4_running(uuid,uuid,date,character,smallint,text,bigint,timestamptz)'::regprocedure, 'complete_reader_summary_daily_canonical_recovery_v4(uuid,uuid,date,character,smallint,text,bigint,timestamptz,bytea,character,jsonb,bytea,character,bytea,character)'::regprocedure, 'read_reader_summary_daily_canonical_recovery_v4_finalized(uuid,uuid)'::regprocedure, 'read_reader_summary_daily_canonical_recovery_v4_terminals(uuid,uuid)'::regprocedure, 'authorize_reader_summary_daily_canonical_recovery_v4_ambiguity_retry(uuid,uuid,date,character,character,timestamptz)'::regprocedure, 'authorize_reader_summary_daily_canonical_recovery_v4_invalid_product_retry_set(uuid,uuid,character)'::regprocedure, 'read_reader_summary_daily_canonical_recovery_v4_unavailable(uuid,uuid,date)'::regprocedure, 'reconcile_reader_summary_daily_canonical_recovery_v4_expired_invalid_runtime_result(uuid,uuid,date,character,character)'::regprocedure, 'fail_reader_summary_daily_canonical_recovery_v4_runtime_result(uuid,uuid,date,character,smallint,text,bigint)'::regprocedure)
...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    await runReaderSummaryDailyTelemetryRelease({
      ...
      applyTelemetryMigration: async () => { await privileges.runReaderSummaryPublicationBootstrapSql("pre", adminDatabaseUrl, runtimeRole, systemRuntimeRole); for (const migration of [telemetryMigration, defaultAclMigration]) cpSync(join("prisma/migrations", migration), join(workspace.directory, "migrations", migration), { recursive: true }); applyOrderedReaderSummaryMigrations(adminDatabaseUrl, workspace); },
