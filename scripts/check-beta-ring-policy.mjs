...
const requiredGatesByRing = new Map([
  [
    ...
    [
      ...
      "check:hn-smoke",
      "check:github-smoke",
      "check:github-repo-radar-smoke",
      "check:github-trending-page-smoke",
      "check:github-trending-page-html-e2e",
      "check:reddit-smoke",
      "check:rss-smoke",
...
for (const ring of rings) {
  ...
  for (const field of [
    ...
    if (!Number.isInteger(ring[field]) || ring[field] < 1) {
      violations.push(
        `${policyPath}: ring "${ring.ringId}" must define positive integer ${field}`,
      );
    }
  }
  ...
  if (ring.maxUsers < previousMaxUsers) {
    violations.push(
      `${policyPath}: ring "${ring.ringId}" cannot reduce maxUsers versus previous ring`,
    );
  }
  previousMaxUsers = ring.maxUsers;

  if (ring.maxEnabledSourcesPerTopic > 5) {
    violations.push(
      `${policyPath}: ring "${ring.ringId}" maxEnabledSourcesPerTopic must stay <= 5 for MVP`,
    );
  }
  ...
  for (const gate of requiredGates) {
    if (!configuredGates.has(gate)) {
      violations.push(
        `${policyPath}: ring "${ring.ringId}" missing required gate "${gate}"`,
      );
    }
  }
  ...
  for (const gate of configuredGates) {
    ...
    if (!scripts[scriptName]) {
      violations.push(
        `${policyPath}: ring "${ring.ringId}" references missing npm script "${scriptName}"`,
      );
    }
    if (!verifyScript.includes(`npm run ${scriptName}`)) {
      violations.push(
        `${packagePath}: npm run verify must include ring gate "${scriptName}"`,
      );
    }
  }
}
...
for (const [field, value] of Object.entries(policy.capacityEvidence ?? {})) {
  if (typeof value !== "string" || value.trim().length === 0) {
    violations.push(
      `${policyPath}: capacityEvidence.${field} must be a non-empty string`,
    );
    continue;
  }
  ...
  if (value.startsWith("ops/") && !existsSync(value)) {
    violations.push(
      `${policyPath}: capacityEvidence.${field} references missing file "${value}"`,
    );
  }
  ...
  if (value.startsWith("npm run ")) {
    ...
    if (!scripts[scriptName]) {
      violations.push(
        `${policyPath}: capacityEvidence.${field} references missing npm script "${scriptName}"`,
      );
    }
  }
}
...
if ((policy.sourceHealthThresholds?.maxProviderFailureRatePercent ?? 101) > 5) {
  violations.push(
    `${policyPath}: maxProviderFailureRatePercent must stay <= 5`,
  );
}
...
if (
  policy.sourceHealthThresholds?.unsupportedSourceBindingPolicy !==
  "reject_binding_capture_source_request_feedback"
) {
  violations.push(
    `${policyPath}: unsupported source policy must reject binding and capture source_request feedback`,
  );
}

if (
  (policy.costThresholds?.maxSummaryEstimatedCostUsdPerWorkspacePerDay ?? 0) <=
  0
) {
  violations.push(
    `${policyPath}: maxSummaryEstimatedCostUsdPerWorkspacePerDay must be positive`,
  );
}
...
for (const requiredAction of [
  ...
  if (!degradationActions.has(requiredAction)) {
    violations.push(
      `${policyPath}: degradationActions missing "${requiredAction}"`,
    );
  }
}
...
for (const forbiddenAction of [
  ...
  if (!forbiddenFirstActions.has(forbiddenAction)) {
    violations.push(
      `${policyPath}: forbiddenFirstActions missing "${forbiddenAction}"`,
    );
  }
}
