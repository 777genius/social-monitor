...
const externalReadinessPath =
  "ops/release/external-beta-readiness-contract.json";
const baselinePath = "ops/release/release-baseline-contract.json";
const dockerStagingReliabilityCapturePath =
  "scripts/capture-docker-staging-reliability-evidence.mjs";
const dockerDurableBackendE2eCapturePath =
  "scripts/capture-docker-durable-backend-e2e-loop.mjs";
const durableBackendE2eCapturePath =
  "scripts/capture-durable-backend-e2e-loop.ts";
const currentScriptPath = fileURLToPath(import.meta.url);
const backendStagingEvidenceBundlePath =
  process.env.BACKEND_STAGING_EVIDENCE_BUNDLE_PATH?.trim();
...
const requiredEvidenceShapeBySignalId = new Map([
  ...
  [
    ...
    [
      ...
      ["readerBrief.sourceMixProviderKeys", "non_empty_string_array"],
      ["readerBrief.citationProviderKeys", "non_empty_string_array"],
      ["readerBrief.qualityStatus", "non_empty_string"],
    ],
  ],
  [
    "backend-loop-scheduled-scan",
...
const forbiddenRealArtifactMarkerPattern =
  /(?:^|[-_:.\s])(example|fixture|synthetic|mock|test)(?:$|[-_:.\s])/i;
...
const forbiddenArtifactValuePatterns = [
  {
    ...
    regex:
      /\b(?:access_token|refresh_token|id_token|api_key|apikey|client_secret|signature|sig)=([^&\s"']+)/gi,
  },
  {
    label: "header credential",
    regex:
      /\b(?:authorization|x-api-key|x-amz-security-token):\s*([^,\s"'}]+)/gi,
  },
  {
...
if (evidence.frontendPolicy !== "deferred_contract_only") {
  violations.push(
    `${evidencePath}: frontendPolicy must keep frontend deferred`,
  );
}

if (
  evidence.externalBetaStatus !== "hold_until_real_staging_drill_output" &&
  evidence.externalBetaStatus !== "passed"
) {
  violations.push(
    `${evidencePath}: externalBetaStatus must be hold_until_real_staging_drill_output or passed`,
  );
}
...
for (const artifact of evidence.stagingEvidenceArtifacts ?? []) {
  if (artifactById.has(artifact.artifactId)) {
    violations.push(
      `${evidencePath}: duplicate staging artifact "${artifact.artifactId}"`,
    );
  }
  ...
  if (!requiredArtifactIds.has(artifact.artifactId)) {
    violations.push(
      `${evidencePath}: unsupported staging artifact "${artifact.artifactId}"`,
    );
  }
  if (!allowedStatuses.has(artifact.status)) {
    violations.push(
      `${evidencePath}: artifact "${artifact.artifactId}" has unsupported status "${artifact.status}"`,
    );
  }
  if (artifact.requiredForExternalBeta !== true) {
    violations.push(
      `${evidencePath}: artifact "${artifact.artifactId}" must be required for external beta`,
    );
  }
  ...
  if (artifact.status === "pending_staging_evidence") {
    for (const field of [
      ...
      if (artifact[field] !== null) {
        violations.push(
          `${evidencePath}: pending artifact "${artifact.artifactId}" must keep ${field}=null`,
        );
      }
    }
  } else {
...
for (const artifactId of requiredArtifactIds) {
  if (!artifactById.has(artifactId)) {
    violations.push(
      `${evidencePath}: missing staging artifact "${artifactId}"`,
    );
  }
}
...
for (const signal of evidence.requiredSignals ?? []) {
  ...
  const artifactSignals =
    signalsByArtifactId.get(signal.stagingArtifactId) ?? [];
  ...
  if (!requiredDomains.has(signal.domain)) {
    violations.push(
      `${evidencePath}: signal "${signal.signalId}" has unsupported domain "${signal.domain}"`,
    );
  }
  if (!allowedStatuses.has(signal.status)) {
    violations.push(
      `${evidencePath}: signal "${signal.signalId}" has unsupported status "${signal.status}"`,
    );
  }
  if (signal.requiredForExternalBeta !== true) {
    violations.push(
      `${evidencePath}: signal "${signal.signalId}" must be required for external beta`,
    );
  }
  ...
  const scriptName = String(signal.verificationCommand ?? "").replace(
    /^npm run /,
    "",
  );
  if (!scripts[scriptName]) {
    violations.push(
      `${evidencePath}: signal "${signal.signalId}" references missing script "${scriptName}"`,
    );
  }
  if (!existsSync(signal.contractArtifact ?? "")) {
    violations.push(
      `${evidencePath}: signal "${signal.signalId}" references missing contract artifact "${signal.contractArtifact}"`,
    );
  }
  if (!artifactById.has(signal.stagingArtifactId)) {
    violations.push(
      `${evidencePath}: signal "${signal.signalId}" references unknown stagingArtifactId "${signal.stagingArtifactId}"`,
    );
  }
  if (
    typeof signal.requiredSignal !== "string" ||
    signal.requiredSignal.trim().length === 0
  ) {
    violations.push(
      `${evidencePath}: signal "${signal.signalId}" must define requiredSignal`,
    );
  }
  if (
    typeof signal.exitCondition !== "string" ||
    signal.exitCondition.trim().length === 0
  ) {
    violations.push(
      `${evidencePath}: signal "${signal.signalId}" must define exitCondition`,
    );
  }
  ...
  if (signal.status === "passed" && artifact?.status !== "passed") {
    violations.push(
      `${evidencePath}: passed signal "${signal.signalId}" requires passed artifact "${signal.stagingArtifactId}"`,
    );
  }
}
...
for (const [artifactId, signals] of signalsByArtifactId.entries()) {
  ...
  if (pendingSignals.length > 0) {
    violations.push(
      `${evidencePath}: passed artifact "${artifactId}" requires passed signals ${pendingSignals.join(", ")}`,
    );
  }
...
if (evidence.externalBetaStatus === "passed" && hasPendingSignal) {
  violations.push(
    `${evidencePath}: externalBetaStatus cannot be passed while staging signals are pending`,
  );
}
...
function validatePassedArtifactContentSchema() {
  ...
  if (!isRecord(schema)) {
    violations.push(
      `${evidencePath}: passedArtifactContentSchema must define the required passed artifact format`,
    );
    return;
  }
  if (schema.schemaVersion !== 1) {
    violations.push(
      `${evidencePath}: passedArtifactContentSchema.schemaVersion must be 1`,
    );
  }
  if (schema.format !== stagingArtifactFormat) {
    violations.push(
      `${evidencePath}: passedArtifactContentSchema.format must be ${stagingArtifactFormat}`,
    );
  }
  if (
    typeof schema.exampleArtifactPath !== "string" ||
    schema.exampleArtifactPath.trim().length === 0
  ) {
    violations.push(
      `${evidencePath}: passedArtifactContentSchema.exampleArtifactPath must reference fixture examples`,
    );
  } else if (!existsSync(schema.exampleArtifactPath)) {
    violations.push(
      `${evidencePath}: passedArtifactContentSchema.exampleArtifactPath does not exist`,
    );
  }
  ...
  for (const field of [
    ...
    if (!requiredTopLevelFields.has(field)) {
      violations.push(
        `${evidencePath}: passedArtifactContentSchema.requiredTopLevelFields must include ${field}`,
      );
    }
  }
  ...
  for (const requirement of [
    ...
    if (!redactionRequirements.has(requirement)) {
      violations.push(
        `${evidencePath}: passedArtifactContentSchema.redactionRequirements must include ${requirement}`,
      );
    }
  }

  if (
    !Array.isArray(schema.signalResultRequirements) ||
    schema.signalResultRequirements.length < 4
  ) {
    violations.push(
      `${evidencePath}: passedArtifactContentSchema.signalResultRequirements must describe signal coverage`,
    );
  } else {
    ...
    for (const requirement of requiredSignalResultRequirementTexts) {
      if (!signalResultRequirements.has(requirement)) {
        violations.push(
          `${evidencePath}: passedArtifactContentSchema.signalResultRequirements must include "${requirement}"`,
        );
      }
    }
  }
...
function requirePassedArtifact(artifact) {
  if (
    ...
    violations.push(
      `${evidencePath}: passed artifact "${artifact.artifactId}" must reference an existing path`,
    );
    return;
  }
  ...
  if (artifact.sha256 !== digest) {
    violations.push(
      `${evidencePath}: passed artifact "${artifact.artifactId}" sha256 must match ${artifact.path}`,
    );
  }
  ...
  for (const field of [
    ...
    if (
      ...
      artifact[field].trim().length === 0
    ) {
      violations.push(
        `${evidencePath}: passed artifact "${artifact.artifactId}" must define ${field}`,
      );
    }
  }
  ...
  if (!/^sha256:[0-9a-f]{64}$/.test(String(artifact.imageDigest ?? ""))) {
    violations.push(
      `${evidencePath}: passed artifact "${artifact.artifactId}" must define immutable imageDigest`,
    );
  }
  validateRealArtifactIdentity(
    artifact,
    `${evidencePath}: stagingEvidenceArtifacts.${artifact.artifactId}`,
  );
}
...
function validatePassedArtifactContent(artifact, signals) {
  ...
  validateArtifactProvenance(content.provenance, artifact.path, {
    allowFixture: false,
  });
  ...
  for (const field of [
    ...
    if (content[field] !== artifact[field]) {
      violations.push(
        `${artifact.path}: ${field} must match ${evidencePath} artifact metadata`,
      );
    }
  }
...
function validateExampleArtifactFixtures(signalsByArtifactId) {
  ...
  for (const example of fixture.examples) {
    ...
    if (examplesByArtifactId.has(example.artifactId)) {
      violations.push(
        `${examplePath}: duplicate example artifactId "${example.artifactId}"`,
      );
    }
  ...
  for (const artifactId of requiredArtifactIds) {
    ...
    validateArtifactProvenance(example.provenance, artifactLabel, {
      allowFixture: true,
    });
...
function validateArtifactContentEnvelope(content, artifactPath, artifactId) {
  ...
  if (
    typeof content.environmentId !== "string" ||
    content.environmentId.trim().length === 0
  ) {
    violations.push(`${artifactPath}: environmentId must be non-empty`);
  }
  if (!/^sha256:[0-9a-f]{64}$/.test(String(content.imageDigest ?? ""))) {
    violations.push(
      `${artifactPath}: imageDigest must be immutable sha256:<64 hex chars>`,
    );
  }
...
function validateEnvironmentArtifacts(signalsByArtifactId) {
  for (const [artifactId, config] of requiredEnvArtifacts.entries()) {
    ...
    validateArtifactProvenance(content.provenance, artifactLabel, {
      allowFixture: false,
    });
...
function validateEnvArtifactMetadata(content, config, artifactLabel) {
  ...
  for (const envVar of config.requiredEnv) {
    ...
    if (
      envVar === "STAGING_ENVIRONMENT_ID" &&
      content.environmentId !== expected
    ) {
      violations.push(
        `${artifactLabel}: environmentId must match STAGING_ENVIRONMENT_ID`,
      );
    }
    if (envVar === "BACKEND_IMAGE_DIGEST" && content.imageDigest !== expected) {
      violations.push(
        `${artifactLabel}: imageDigest must match BACKEND_IMAGE_DIGEST`,
      );
    }
    if (envVar === "API_BASE_URL") {
      ...
      if (!isHttpUrlWithoutCredentials(content.apiBaseUrl)) {
        violations.push(
          `${artifactLabel}: apiBaseUrl must be an http(s) URL without credentials`,
        );
      }
    }
  }
...
function expectedEnvValueForArtifact(content, config, envVar) {
  ...
  if (
    backendStagingEvidenceBundlePath === undefined ||
    backendStagingEvidenceBundlePath.length === 0
  ) {
    return undefined;
  }
  ...
  try {
    bundle = JSON.parse(
      readPrivateEvidenceJsonFile(
        backendStagingEvidenceBundlePath,
  ...
  const artifactPath = readOptionalEnv(config.envVar);
  const bundleArtifact = (bundle.artifacts ?? []).find(
    (item) => item?.artifactId === content.artifactId,
  );
  if (bundleArtifact?.path !== artifactPath) {
    violations.push(
      `BACKEND_STAGING_EVIDENCE_BUNDLE_PATH must reference ${config.envVar} to cover API_BASE_URL`,
    );
    return undefined;
  }
  if (bundle.imageDigest !== content.imageDigest) {
    violations.push(
      `BACKEND_STAGING_EVIDENCE_BUNDLE_PATH imageDigest must match ${config.envVar} imageDigest`,
    );
  }

  const apiBaseUrl =
    typeof content.apiBaseUrl === "string" ? content.apiBaseUrl.trim() : "";
...
function validateNoRealArtifactFixtureMarkers(value, label, path = []) {
  if (typeof value === "string") {
    ...
    if (marker !== null) {
      ...
      violations.push(
        `${label}: ${fieldPath} must not contain fixture marker "${marker[1]}"`,
      );
    }
    return;
  }
  ...
  if (Array.isArray(value)) {
    for (const [index, item] of value.entries()) {
      validateNoRealArtifactFixtureMarkers(item, label, [
        ...path,
        `[${index}]`,
      ]);
    }
...
function validateEnvArtifactValidation(validationRules) {
  if (!Array.isArray(validationRules)) {
    violations.push(
      `${evidencePath}: passedArtifactContentSchema.envArtifactValidation must be an array`,
    );
    return;
  }
  ...
  for (const [index, rule] of validationRules.entries()) {
    ...
    if (rule.envVar !== expected.envVar) {
      violations.push(
        `${evidencePath}: ${label}.envVar must be ${expected.envVar}`,
      );
    }
    if (rule.format !== stagingArtifactFormat) {
      violations.push(
        `${evidencePath}: ${label}.format must be ${stagingArtifactFormat}`,
      );
    }
    for (const envVar of expected.requiredEnv) {
      if (!rule.requiredEnv?.includes(envVar)) {
        violations.push(
          `${evidencePath}: ${label}.requiredEnv must include ${envVar}`,
        );
      }
    }
  }
  ...
  for (const artifactId of requiredEnvArtifacts.keys()) {
    if (!seenRules.has(artifactId)) {
      violations.push(
        `${evidencePath}: envArtifactValidation must include ${artifactId}`,
      );
    }
  }
}
...
function readArtifactContent(artifact, options = {}) {
  try {
    const rawContent =
      options.rawContent ?? readFileSync(artifact.path, "utf8");
    validateNoSensitiveArtifactContent(rawContent, artifact.path);
    const content = JSON.parse(rawContent);
    if (!isRecord(content)) {
      violations.push(
        `${artifact.path}: passed staging artifact must be a JSON object`,
      );
      return undefined;
    }
    ...
    violations.push(
      `${artifact.path}: passed staging artifact must be valid JSON (${message})`,
    );
    return undefined;
  }
}
...
function validateNoSensitiveArtifactContent(content, artifactPath) {
  ...
  for (const fragment of forbiddenArtifactFragments) {
    if (serialized.includes(fragment)) {
      violations.push(
        `${artifactPath}: passed staging artifact must not contain sensitive fragment "${fragment}"`,
      );
    }
  }
  validateNoSensitivePatterns(
    content,
    `${artifactPath}: passed staging artifact`,
  );
}
...
function validateSignalResults(content, artifact, signals) {
  ...
  for (const result of content.signalResults) {
    if (!isRecord(result)) {
      violations.push(
        `${artifact.path}: every signalResults item must be an object`,
      );
      continue;
    }
    if (resultBySignalId.has(result.signalId)) {
      violations.push(
        `${artifact.path}: duplicate signal result "${result.signalId}"`,
      );
    }
    resultBySignalId.set(result.signalId, result);
    if (!expectedSignalIds.has(result.signalId)) {
      violations.push(
        `${artifact.path}: unsupported signal result "${result.signalId}" for ${artifact.artifactId}`,
      );
    }
    if (result.status !== "passed") {
      violations.push(
        `${artifact.path}: signal result "${result.signalId}" must have status=passed`,
      );
    }
    if (
      typeof result.observedAt !== "string" ||
      result.observedAt.trim().length === 0
    ) {
      violations.push(
        `${artifact.path}: signal result "${result.signalId}" must define observedAt`,
      );
    } else if (!matchesEvidenceType(result.observedAt, "iso_timestamp")) {
      violations.push(
        `${artifact.path}: signal result "${result.signalId}" observedAt must be an ISO timestamp`,
      );
    } else {
    ...
    if (!hasNonEmptyEvidence(result.evidence)) {
      violations.push(
        `${artifact.path}: signal result "${result.signalId}" must include non-empty evidence`,
      );
    } else {
  ...
  for (const signal of signals) {
    if (!resultBySignalId.has(signal.signalId)) {
      violations.push(
        `${artifact.path}: missing signal result "${signal.signalId}" for ${artifact.artifactId}`,
      );
    }
  }
}
...
function validateSignalEvidenceSchemaMap() {
  for (const signalId of requiredSignalIds) {
    ...
    if (!Array.isArray(shape) || shape.length === 0) {
      violations.push(
        `${evidencePath}: missing signal-specific evidence schema for "${signalId}"`,
      );
      continue;
    }
    ...
    for (const [fieldPath, fieldType] of shape) {
      if (typeof fieldPath !== "string" || fieldPath.trim().length === 0) {
        violations.push(
          `${evidencePath}: evidence schema for "${signalId}" has an empty field path`,
        );
      }
      if (seenFields.has(fieldPath)) {
        violations.push(
          `${evidencePath}: evidence schema for "${signalId}" duplicates field "${fieldPath}"`,
        );
      }
      seenFields.add(fieldPath);
      if (!isSupportedEvidenceType(fieldType)) {
        violations.push(
          `${evidencePath}: evidence schema for "${signalId}" uses unsupported type "${fieldType}"`,
        );
      }
    }
  }
  ...
  for (const signalId of requiredEvidenceShapeBySignalId.keys()) {
    if (!requiredSignalIds.has(signalId)) {
      violations.push(
        `${evidencePath}: evidence schema references unsupported signal "${signalId}"`,
      );
    }
  }
}
...
function validateSignalEvidenceShape(artifactPath, result) {
  ...
  if (shape === undefined) {
    violations.push(
      `${artifactPath}: signal result "${result.signalId}" has no evidence schema`,
    );
    return;
  }
  if (!isRecord(result.evidence)) {
    violations.push(
      `${artifactPath}: signal result "${result.signalId}" evidence must be an object`,
    );
    return;
  }
  ...
  for (const [fieldPath, fieldType] of shape) {
    ...
    if (value === undefined) {
      violations.push(
        `${artifactPath}: signal result "${result.signalId}" evidence must include ${fieldPath}`,
      );
      continue;
    }
  ...
  if (result.signalId === "backend-loop-reader-brief") {
    validateReaderBriefProviderCoverage(artifactPath, result.evidence);
  }
  ...
  switch (fieldType) {
    ...
    case "scheduled_idempotency_key":
      return (
        ...
        value.trim().length > "scheduled:".length
      );
    case "auto_summary_idempotency_key":
      return (
        typeof value === "string" &&
        value.startsWith("auto-summary:") &&
        value.trim().length > "auto-summary:".length
      );
...
function validateDurableBackendProviderCoverage(artifactPath, evidenceValue) {
  ...
  for (const providerKey of [
    ...
    if (!providerKeys.has(providerKey)) {
      violations.push(
        `${artifactPath}: signal result "backend-loop-topic-to-delivery-audit" must include ${providerKey} provider feed evidence`,
      );
    }
  }
}

function validateReaderBriefProviderCoverage(artifactPath, evidenceValue) {
  const sourceMixProviderKeys = getPath(
    evidenceValue,
    "readerBrief.sourceMixProviderKeys",
  );
  const citationProviderKeys = getPath(
    evidenceValue,
    "readerBrief.citationProviderKeys",
  );
  for (const providerKey of [
    "github-issues",
    "github-trending-page",
    ...
    if (
      Array.isArray(sourceMixProviderKeys) &&
      !sourceMixProviderKeys.includes(providerKey)
    ) {
      violations.push(
        `${artifactPath}: signal result "backend-loop-reader-brief" must include ${providerKey} in readerBrief.sourceMixProviderKeys`,
      );
    }
    if (
      Array.isArray(citationProviderKeys) &&
      !citationProviderKeys.includes(providerKey)
    ) {
      violations.push(
        `${artifactPath}: signal result "backend-loop-reader-brief" must include ${providerKey} in readerBrief.citationProviderKeys`,
      );
    }
  }
}
...
function validateScheduledScanEvidence(artifactPath, evidenceValue) {
  const completedAtMs = parseIsoTimestampMs(
    getPath(evidenceValue, "scheduledScan.completedAt"),
  );
  const nextRunAtMs = parseIsoTimestampMs(
    getPath(evidenceValue, "scheduledScan.nextRunAtAfterSchedule"),
  );
  if (
    completedAtMs !== undefined &&
    nextRunAtMs !== undefined &&
    nextRunAtMs <= completedAtMs
  ) {
    violations.push(
      `${artifactPath}: signal result "backend-loop-scheduled-scan" evidence.scheduledScan.nextRunAtAfterSchedule must be after completedAt`,
    );
  }
}

function validateAutoSummaryEvidence(artifactPath, evidenceValue) {
  const latestFeedItemObservedAtMs = parseIsoTimestampMs(
    getPath(evidenceValue, "autoSummary.latestFeedItemObservedAt"),
  );
  const requestedAtMs = parseIsoTimestampMs(
    getPath(evidenceValue, "autoSummary.requestedAt"),
  );
  const completedAtMs = parseIsoTimestampMs(
    getPath(evidenceValue, "autoSummary.completedAt"),
  );
  if (
    latestFeedItemObservedAtMs !== undefined &&
    requestedAtMs !== undefined &&
    requestedAtMs < latestFeedItemObservedAtMs
  ) {
    violations.push(
      `${artifactPath}: signal result "backend-loop-auto-summary-scheduler" evidence.autoSummary.requestedAt must be after latestFeedItemObservedAt`,
    );
  }
  if (
    requestedAtMs !== undefined &&
    ...
    violations.push(
      `${artifactPath}: signal result "backend-loop-auto-summary-scheduler" evidence.autoSummary.completedAt must be after requestedAt`,
    );
  }
}
...
function validateRabbitMqAckEvidence(artifactPath, evidenceValue) {
  ...
  if (Number.isInteger(before) && Number.isInteger(after) && after >= before) {
    violations.push(
      `${artifactPath}: signal result "rabbitmq-consumer-ack" evidence.queueDepthAfterAck must be lower than queueDepthBeforeAck`,
    );
  }
}
...
function validatePostgresReplayStateEvidence(artifactPath, evidenceValue) {
  if (
    evidenceValue.countsMatched === true &&
    !objectsEqual(evidenceValue.beforeCounts, evidenceValue.afterCounts)
  ) {
    violations.push(
      `${artifactPath}: signal result "postgres-outbox-inbox-idempotency" countsMatched=true requires beforeCounts and afterCounts to match`,
    );
  }
  if (
    evidenceValue.fingerprintsMatched === true &&
    !objectsEqual(
      evidenceValue.beforeFingerprints,
      evidenceValue.afterFingerprints,
    )
  ) {
    violations.push(
      `${artifactPath}: signal result "postgres-outbox-inbox-idempotency" fingerprintsMatched=true requires beforeFingerprints and afterFingerprints to match`,
    );
  }
}
...
function validatePostgresNoDuplicateEvidence(artifactPath, evidenceValue) {
  ...
  if (Number.isInteger(before) && Number.isInteger(after) && before !== after) {
    violations.push(
      `${artifactPath}: signal result "postgres-no-duplicate-side-effects" duplicate probe counts must remain stable`,
    );
  }
  if (evidenceValue.duplicateInsertSuppressed === true && before !== 1) {
    violations.push(
      `${artifactPath}: signal result "postgres-no-duplicate-side-effects" duplicate probe must start from exactly one delivery attempt`,
    );
  }
  if (
    !objectsEqual(
      evidenceValue.beforeResumeCounts,
      evidenceValue.afterResumeCounts,
    )
  ) {
    violations.push(
      `${artifactPath}: signal result "postgres-no-duplicate-side-effects" beforeResumeCounts and afterResumeCounts must match`,
    );
  }
}
...
function validateArtifactTimeWindow(content, artifactPath) {
  ...
  if (completedAtMs < startedAtMs) {
    violations.push(
      `${artifactPath}: completedAt must be greater than or equal to startedAt`,
    );
  }
}
...
function validateSignalObservedAtWindow(artifact, result) {
  ...
  if (observedAtMs < startedAtMs || observedAtMs > completedAtMs) {
    violations.push(
      `${artifact.path}: signal result "${result.signalId}" observedAt must be within artifact startedAt/completedAt`,
    );
  }
}
...
function requirePackageWiring() {
  ...
  if (
    !String(
      scripts["capture:docker-staging-reliability-evidence"] ?? "",
    ).includes(dockerStagingReliabilityCapturePath)
  ) {
    violations.push(
      `${packagePath}: capture:docker-staging-reliability-evidence must run ${dockerStagingReliabilityCapturePath}`,
    );
  }
  if (
    !String(scripts["capture:docker-durable-backend-e2e-loop"] ?? "").includes(
      dockerDurableBackendE2eCapturePath,
    )
  ) {
    violations.push(
      `${packagePath}: capture:docker-durable-backend-e2e-loop must run ${dockerDurableBackendE2eCapturePath}`,
    );
  }
  if (
    !String(scripts["capture:durable-backend-e2e-loop"] ?? "").includes(
      durableBackendE2eCapturePath,
    )
  ) {
    violations.push(
      `${packagePath}: capture:durable-backend-e2e-loop must run ${durableBackendE2eCapturePath}`,
    );
  }

  if (!new Set(backendSafe.backendScripts ?? []).has(gateScript)) {
    violations.push(
      `${backendSafePath}: backend-safe verify must include ${gateScript}`,
    );
  }
}
...
function requireCaptureScriptWiring() {
  const captureSource = readFileSync(
    dockerStagingReliabilityCapturePath,
    "utf8",
  );
  for (const marker of [
    ...
    if (!captureSource.includes(marker)) {
      violations.push(
        `${dockerStagingReliabilityCapturePath}: capture must include ${marker}`,
      );
    }
  }

  const e2eCaptureSource = readFileSync(
    dockerDurableBackendE2eCapturePath,
    "utf8",
  );
  for (const marker of [
    ...
    if (!e2eCaptureSource.includes(marker)) {
      violations.push(
        `${dockerDurableBackendE2eCapturePath}: capture must include ${marker}`,
      );
    }
  }

  const durableBackendE2eCaptureSource = readFileSync(
    durableBackendE2eCapturePath,
    "utf8",
  );
  for (const marker of [
    ...
    if (!durableBackendE2eCaptureSource.includes(marker)) {
      violations.push(
        `${durableBackendE2eCapturePath}: capture must include ${marker}`,
      );
    }
  }
  ...
  for (const marker of [
    ...
    if (!selfSource.includes(marker)) {
      violations.push(
        `${currentScriptPath}: direct staging artifact env path guard must include ${marker}`,
      );
    }
  }
}
...
function validateCaptureOutputPathGuards() {
  const rabbitWorkspaceArtifactPath = resolve(
    "rabbitmq-staging-drill-workspace-output.json",
  );
  const rabbitResult = runDockerStagingCaptureExpectingFailure({
    ...
    POSTGRES_RESTORE_DRILL_ARTIFACT_PATH:
      "/tmp/social-monitor-postgres-restore-drill-output.json",
    STAGING_RELIABILITY_ENV_PATH: "/tmp/social-monitor-staging-reliability.env",
  });
  if (rabbitResult.exitCode === 0) {
    violations.push(
      `${dockerStagingReliabilityCapturePath}: capture must reject workspace RABBITMQ_STAGING_DRILL_ARTIFACT_PATH`,
    );
  } else if (
    !rabbitResult.output.includes(
      "RABBITMQ_STAGING_DRILL_ARTIFACT_PATH must not write release evidence into the git workspace",
    )
  ) {
    violations.push(
      `${dockerStagingReliabilityCapturePath}: workspace RabbitMQ artifact path rejection must explain evidence path policy`,
    );
  }
  if (existsSync(rabbitWorkspaceArtifactPath)) {
    violations.push(
      `${dockerStagingReliabilityCapturePath}: workspace RabbitMQ artifact path rejection must not create ${rabbitWorkspaceArtifactPath}`,
    );
  }

  const postgresWorkspaceArtifactPath = resolve(
    "postgres-restore-drill-workspace-output.json",
  );
  const postgresResult = runDockerStagingCaptureExpectingFailure({
    RABBITMQ_STAGING_DRILL_ARTIFACT_PATH:
      "/tmp/social-monitor-rabbitmq-staging-drill-output.json",
    POSTGRES_RESTORE_DRILL_ARTIFACT_PATH: postgresWorkspaceArtifactPath,
    STAGING_RELIABILITY_ENV_PATH: "/tmp/social-monitor-staging-reliability.env",
  });
  if (postgresResult.exitCode === 0) {
    violations.push(
      `${dockerStagingReliabilityCapturePath}: capture must reject workspace POSTGRES_RESTORE_DRILL_ARTIFACT_PATH`,
    );
  } else if (
    !postgresResult.output.includes(
      "POSTGRES_RESTORE_DRILL_ARTIFACT_PATH must not write release evidence into the git workspace",
    )
  ) {
    violations.push(
      `${dockerStagingReliabilityCapturePath}: workspace Postgres artifact path rejection must explain evidence path policy`,
    );
  }
  if (existsSync(postgresWorkspaceArtifactPath)) {
    violations.push(
      `${dockerStagingReliabilityCapturePath}: workspace Postgres artifact path rejection must not create ${postgresWorkspaceArtifactPath}`,
    );
  }

  const durableBackendWorkspaceArtifactPath = resolve(
    "durable-backend-e2e-workspace-output.json",
  );
  ...
  if (durableBackendResult.exitCode === 0) {
    violations.push(
      `${dockerDurableBackendE2eCapturePath}: capture must reject workspace DURABLE_BACKEND_E2E_ARTIFACT_PATH`,
    );
  } else if (
    !durableBackendResult.output.includes(
      "DURABLE_BACKEND_E2E_ARTIFACT_PATH must not write release evidence into the git workspace",
    )
  ) {
    violations.push(
      `${dockerDurableBackendE2eCapturePath}: workspace durable backend E2E artifact path rejection must explain evidence path policy`,
    );
  }
  if (existsSync(durableBackendWorkspaceArtifactPath)) {
    violations.push(
      `${dockerDurableBackendE2eCapturePath}: workspace durable backend E2E artifact path rejection must not create ${durableBackendWorkspaceArtifactPath}`,
    );
  }
}
...
function validateDirectArtifactPathGuards() {
  ...
  for (const config of requiredEnvArtifacts.values()) {
    const workspaceArtifactPath = resolve(
      `${config.envVar.toLowerCase().replaceAll("_", "-")}-direct-workspace-output.json`,
    );
    ...
    if (workspaceResult.exitCode === 0) {
      violations.push(
        `check:staging-reliability-evidence must reject workspace ${config.envVar}`,
      );
    } else if (
      !workspaceResult.output.includes(
        `${config.envVar} must not write release evidence into the git workspace`,
      )
    ) {
      violations.push(
        `check:staging-reliability-evidence workspace ${config.envVar} rejection must explain evidence path policy`,
      );
    }
    if (existsSync(workspaceArtifactPath)) {
      violations.push(
        `check:staging-reliability-evidence workspace ${config.envVar} rejection must not create ${workspaceArtifactPath}`,
      );
    }

    const tempDirectory = mkdtempSync(
      join(tmpdir(), "staging-reliability-direct-"),
    );
    try {
      const publicArtifactPath = join(
        tempDirectory,
        "staging-reliability-artifact.json",
      );
      ...
      if (publicModeResult.exitCode === 0) {
        violations.push(
          `check:staging-reliability-evidence must reject public ${config.envVar} permissions`,
        );
      } else if (
        !publicModeResult.output.includes(
          `${config.envVar} must use 0600-style private file permissions`,
        )
      ) {
        violations.push(
          `check:staging-reliability-evidence public ${config.envVar} rejection must explain private file mode policy`,
        );
      }
    } finally {
...
function requireReleaseWiring() {
  const releaseGateIds = new Set(
    (releaseContract.requiredGates ?? []).map((gate) => gate.gateId),
  );
  const releaseGateCommands = new Set(
    (releaseContract.requiredGates ?? []).map((gate) => gate.command),
  );
  ...
  if (!releaseGateCommands.has(gateCommand)) {
    violations.push(
      `${releaseContractPath}: release gates must include ${gateScript}`,
    );
  }
}
...
function requireBackendOpsWiring() {
  for (const domainId of requiredBackendOpsDomains) {
    const domain = (backendOps.requiredDomains ?? []).find(
      (item) => item.domainId === domainId,
    );
    if (domain === undefined) {
      violations.push(
        `${backendOpsPath}: missing backend ops domain "${domainId}"`,
      );
      continue;
    }
    if (!domain.gates?.includes(gateScript)) {
      violations.push(
        `${backendOpsPath}: domain "${domainId}" must include ${gateScript}`,
      );
    }
    if (!domain.releaseGateIds?.includes(gateId)) {
      violations.push(
        `${backendOpsPath}: domain "${domainId}" must include ${gateId} release gate`,
      );
    }
    if (!domain.artifacts?.includes(evidencePath)) {
      violations.push(
        `${backendOpsPath}: domain "${domainId}" must include ${evidencePath}`,
      );
    }
  }
}
...
function requireExternalReadinessWiring() {
  for (const groupId of requiredExternalGroups) {
    const group = (externalReadiness.requiredEvidenceGroups ?? []).find(
      (item) => item.groupId === groupId,
    );
    if (group === undefined) {
      violations.push(
        `${externalReadinessPath}: missing external readiness group "${groupId}"`,
      );
      continue;
    }
    if (!group.verificationCommands?.includes(gateCommand)) {
      violations.push(
        `${externalReadinessPath}: group "${groupId}" must include ${gateScript}`,
      );
    }
    if (!group.requiredArtifacts?.includes(evidencePath)) {
      violations.push(
        `${externalReadinessPath}: group "${groupId}" must include ${evidencePath}`,
      );
    }
  }
}
...
function requireBaselineWiring() {
  if (!baseline.requiredGreenScripts?.includes(gateScript)) {
    violations.push(
      `${baselinePath}: requiredGreenScripts must include ${gateScript}`,
    );
  }
  if (
    !(baseline.trackedArtifacts ?? []).some(
      (artifact) => artifact.path === evidencePath,
    )
  ) {
    violations.push(
      `${baselinePath}: trackedArtifacts must include ${evidencePath}`,
    );
  }
  const examplePath = evidence.passedArtifactContentSchema?.exampleArtifactPath;
  if (
    typeof examplePath === "string" &&
    !(baseline.trackedArtifacts ?? []).some(
      (artifact) => artifact.path === examplePath,
    )
  ) {
    violations.push(
      `${baselinePath}: trackedArtifacts must include ${examplePath}`,
    );
  }
}
