...
if (contract.frontendPolicy !== "deferred_contract_only") {
  violations.push(
    `${contractPath}: frontendPolicy must keep frontend deferred`,
  );
}

if (contract.decision !== "hold") {
  violations.push(
    `${contractPath}: decision must stay hold until all go prerequisites pass`,
  );
}

if (contract.capacityEnvelopeSource !== externalReadinessPath) {
  violations.push(
    `${contractPath}: capacityEnvelopeSource must reference ${externalReadinessPath}`,
  );
}
if (contract.ringPolicy !== betaPolicyPath) {
  violations.push(
    `${contractPath}: ringPolicy must reference ${betaPolicyPath}`,
  );
}
if (contract.decisionRecord !== betaDecisionPath) {
  violations.push(
    `${contractPath}: decisionRecord must reference ${betaDecisionPath}`,
  );
}
...
function validateCapacityEnvelope() {
  ...
  const targetRing = (betaPolicy.rings ?? []).find(
    (ring) => ring.ringId === contract.targetRing,
  );

  if (externalEnvelope.ringId !== contract.targetRing) {
    violations.push(
      `${externalReadinessPath}: capacityEnvelope.ringId must be ${contract.targetRing}`,
    );
  }
  ...
  for (const field of requiredLimitFields) {
    ...
    if (externalEnvelope[field] !== limits[field]) {
      violations.push(
        `${externalReadinessPath}: capacityEnvelope.${field} must match ${contractPath}`,
      );
    }
    if (targetRing !== undefined && targetRing[field] !== limits[field]) {
      violations.push(
        `${betaPolicyPath}: target ring "${contract.targetRing}" ${field} must match capacity envelope`,
      );
    }
  }

  if (limits.maxEnabledSourcesPerTopic > 5) {
    violations.push(
      `${contractPath}: maxEnabledSourcesPerTopic must stay <= 5 for MVP`,
    );
  }
  if (limits.minScheduledScanIntervalMinutes < 60) {
    violations.push(
      `${contractPath}: minScheduledScanIntervalMinutes must stay >= 60 before external beta go`,
    );
  }
  if (limits.maxQueueLagSeconds > 300) {
    violations.push(
      `${contractPath}: maxQueueLagSeconds must stay <= 300 for private beta 1`,
    );
  }
  if (limits.maxSummaryEstimatedCostUsdPerWorkspacePerDay > 5) {
    violations.push(
      `${contractPath}: maxSummaryEstimatedCostUsdPerWorkspacePerDay must stay <= 5`,
    );
  }
  if (targetRing === undefined) {
    violations.push(
      `${betaPolicyPath}: missing target ring "${contract.targetRing}"`,
    );
  }
}
...
function validatePolicyAndDecision() {
  if (externalReadiness.externalBetaDecision !== "hold") {
    violations.push(
      `${externalReadinessPath}: externalBetaDecision must remain hold until go prerequisites pass`,
    );
  }
  if (betaDecision.decision !== "hold") {
    violations.push(
      `${betaDecisionPath}: decision must remain hold until go prerequisites pass`,
    );
  }
  if (betaDecision.candidateNextRing !== contract.targetRing) {
    violations.push(
      `${betaDecisionPath}: candidateNextRing must match ${contract.targetRing}`,
    );
  }

  const holdReasonIds = new Set(
    (betaDecision.holdReasons ?? []).map((reason) => reason.reasonId),
  );
  for (const holdReason of requiredHoldReasons) {
    if (!holdReasonIds.has(holdReason)) {
      violations.push(
        `${betaDecisionPath}: holdReasons missing "${holdReason}"`,
      );
    }
  }
  ...
  for (const reason of betaDecision.holdReasons ?? []) {
    if (typeof reason.owner !== "string" || reason.owner.trim().length === 0) {
      violations.push(
        `${betaDecisionPath}: holdReason "${reason.reasonId}" must define owner`,
      );
    }
    if (
      typeof reason.exitCondition !== "string" ||
      reason.exitCondition.trim().length === 0
    ) {
      violations.push(
        `${betaDecisionPath}: holdReason "${reason.reasonId}" must define exitCondition`,
      );
    }
  }
  ...
  for (const action of [
    ...
    if (!forbiddenWhileHeld.has(action)) {
      violations.push(
        `${contractPath}: forbiddenWhileHeld missing "${action}"`,
      );
    }
  }
}
...
function validateGoPrerequisites() {
  ...
  for (const prerequisite of contract.goPrerequisites ?? []) {
    for (const field of [
      ...
      if (
        ...
        prerequisite[field].trim().length === 0
      ) {
    ...
    if (observedPrerequisiteIds.has(prerequisite.prerequisiteId)) {
      violations.push(
        `${contractPath}: duplicate goPrerequisite "${prerequisite.prerequisiteId}"`,
      );
    }
    observedPrerequisiteIds.add(prerequisite.prerequisiteId);
    if (!requiredGoPrerequisiteIds.has(prerequisite.prerequisiteId)) {
      violations.push(
        `${contractPath}: unsupported goPrerequisite "${prerequisite.prerequisiteId}"`,
      );
    }
    if (!holdReasonIds.has(prerequisite.holdReasonId)) {
      violations.push(
        `${betaDecisionPath}: holdReasons missing go prerequisite reason "${prerequisite.holdReasonId}"`,
      );
    }
    if (!existsSync(prerequisite.artifact)) {
      violations.push(
        `${contractPath}: goPrerequisite "${prerequisite.prerequisiteId}" references missing artifact`,
      );
      continue;
    }
  ...
  for (const requiredPrerequisiteId of requiredGoPrerequisiteIds) {
    if (!observedPrerequisiteIds.has(requiredPrerequisiteId)) {
      violations.push(
        `${contractPath}: goPrerequisites missing "${requiredPrerequisiteId}"`,
      );
    }
  }

  if (
    hasPendingPrerequisite &&
    externalReadiness.externalBetaDecision !== "hold"
  ) {
    violations.push(
      `${externalReadinessPath}: pending go prerequisites require externalBetaDecision=hold`,
    );
  }
  if (hasPendingPrerequisite && betaDecision.decision !== "hold") {
    violations.push(
      `${betaDecisionPath}: pending go prerequisites require decision=hold`,
    );
  }
}
...
function validateLoadCostGuardrails() {
  ...
  if (guardrails.script !== "scripts/check-load-cost.mjs") {
    violations.push(
      `${contractPath}: loadCostGuardrails.script must be scripts/check-load-cost.mjs`,
    );
  }
  if (!existsSync(guardrails.script ?? "")) {
    violations.push(
      `${contractPath}: loadCostGuardrails.script must reference an existing script`,
    );
    return;
  }
  ...
  for (const field of [
    ...
    if (!derivedFromLimits.has(field)) {
      violations.push(
        `${contractPath}: loadCostGuardrails.derivedFromLimits missing "${field}"`,
      );
    }
  }
  if (guardrails.mustRejectNoisyTenant !== true) {
    violations.push(
      `${contractPath}: loadCostGuardrails.mustRejectNoisyTenant must be true`,
    );
  }
  if (guardrails.mustKeepQuietTenantAllowed !== true) {
    violations.push(
      `${contractPath}: loadCostGuardrails.mustKeepQuietTenantAllowed must be true`,
    );
  }
  ...
  if (!scriptSource.includes(contractPath)) {
    violations.push(
      `${guardrails.script}: load/cost script must read ${contractPath}`,
    );
  }
  for (const field of derivedFromLimits) {
    if (!requiredLimitFields.has(field)) {
      violations.push(
        `${contractPath}: loadCostGuardrails references unsupported limit "${field}"`,
      );
    }
    if (!scriptSource.includes(field)) {
      violations.push(
        `${guardrails.script}: load/cost script must reference limit "${field}"`,
      );
    }
  }
}
...
function requireWiring() {
  ...
  const baselineArtifacts = new Set(
    (baseline.trackedArtifacts ?? []).map((artifact) => artifact.path),
  );
  ...
  if (externalDomain === undefined) {
    ...
    if (!externalDomain.gates?.includes(gateScript)) {
      violations.push(
        `${backendOpsPath}: external-beta-evidence domain must include ${gateScript}`,
      );
    }
    if (!externalDomain.releaseGateIds?.includes(gateId)) {
      violations.push(
        `${backendOpsPath}: external-beta-evidence domain must include ${gateId}`,
      );
    }
    if (!externalDomain.artifacts?.includes(contractPath)) {
      violations.push(
        `${backendOpsPath}: external-beta-evidence domain must include ${contractPath}`,
      );
    }
  }
  ...
  if (externalGroup === undefined) {
    violations.push(
      `${externalReadinessPath}: missing capacity-envelope-beta-ring-decision group`,
    );
  } else {
    if (!externalGroup.verificationCommands?.includes(gateCommand)) {
      violations.push(
        `${externalReadinessPath}: capacity-envelope-beta-ring-decision group must include ${gateScript}`,
      );
    }
    if (!externalGroup.requiredArtifacts?.includes(contractPath)) {
      violations.push(
        `${externalReadinessPath}: capacity-envelope-beta-ring-decision group must include ${contractPath}`,
      );
    }
  }
  ...
  if (!baselineArtifacts.has(contractPath)) {
    violations.push(
      `${baselinePath}: trackedArtifacts must include ${contractPath}`,
    );
  }
}
...
function requireRuntimeCapacityEnforcement() {
  ...
  const envExample = readFileSync(
    "ops/release/external-beta-evidence.env.example",
    "utf8",
  );
  ...
  for (const marker of [
    ...
    if (!monitoringModule.includes(marker)) {
      violations.push(
        `${monitoringModulePath}: runtime capacity enforcement missing "${marker}"`,
      );
    }
    if (
      marker.startsWith("MONITORING_") &&
      !envExample.includes(`${marker}=`)
    ) {
      violations.push(
        `ops/release/external-beta-evidence.env.example: missing "${marker}"`,
      );
    }
  }
  ...
  for (const [field, expectedValue] of [
    ...
    if (!capacityLimits.includes(`${field}: ${expectedValue}`)) {
      violations.push(
        `${capacityLimitsPath}: ${field} must default to capacity envelope value ${expectedValue}`,
      );
    }
  }
  ...
  for (const marker of [
    ...
    if (!bindSource.includes(marker)) {
      violations.push(
        `${bindSourcePath}: source binding capacity enforcement missing "${marker}"`,
      );
    }
  }
}
