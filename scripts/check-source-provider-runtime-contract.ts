...
type ProviderRuntimeRequirement = {
  readonly credentialMode: string;
  readonly signals: readonly string[];
};

const contractPath = "ops/ingestion/source-provider-runtime-contract.json";
const externalBetaEvidenceRunnerPath =
  "ops/release/external-beta-evidence-runner.json";
const requiredProviders: ReadonlyMap<string, ProviderRuntimeRequirement> =
  new Map([
    [
      "hacker-news",
      {
...
const optionalDisabledProviders: ReadonlyMap<
  string,
...
const contractProviders: ReadonlyMap<string, ProviderRuntimeRequirement> =
  new Map([...requiredProviders, ...optionalDisabledProviders] as const);
...
function validateContract(): void {
  ...
  const optionalDisabledProviderKeys = Array.from(
    optionalDisabledProviders.keys(),
  );
  const allowedContractProviderKeys = [
    ...requiredProviderKeys,
    ...optionalDisabledProviderKeys,
  ];
  ...
  for (const [providerKey, requirement] of contractProviders) {
...
async function verifyGitHubTrendingPageRuntime(): Promise<void> {
  ...
  await withMockedFetch(
    async (input, init) => {
      ...
      assert(
        readHeader(init?.headers, "accept") ===
...
async function verifyHackerNewsRuntime(): Promise<void> {
  ...
  const sourceProvider = new HackerNewsSourceProvider(new HttpHackerNewsClient(), new SystemClock());
  assertFailure(
    sourceProvider,
