...
- Dependencies point inward: `interfaces -> features -> ports/domain`, while `adapters -> ports`; domain/features never import adapters.
...
- Feature use cases depend on ports and orchestrate behavior through domain objects. They must not instantiate concrete repositories, queues, HTTP clients, clocks, id generators or in-memory adapters.
...
- Adapters translate external details into ports. They may use Prisma, RabbitMQ, SDKs, HTTP clients and crypto, but their types must not leak inward.
...
- Ports are inner-boundary abstractions. They must not import frameworks, adapters, public contracts or read `process.env`.
- Adapters must receive runtime configuration explicitly from composition roots. Do not read `process.env` from adapters, ports, domain or feature code.
...
- Production runtime imports must not form runtime circular dependencies. Split ports, domain primitives or mappers instead of importing back across a layer.
- Adapters and feature use cases must receive `Clock`/ID helpers explicitly when behavior depends on current time or generated identity. Do not hide wall-clock reads behind `Date.now()`, zero-argument `new Date()` or adapter defaults that instantiate `SystemClock`/`CryptoIdGenerator`.
...
- OCP: add providers, delivery channels, source adapters and model adapters by implementing ports/registries, not by editing long conditional chains in use cases.
- LSP: fake/in-memory/test adapters must honor the same port contract as durable adapters. They cannot silently skip tenant scope, idempotency, status transitions or cursor semantics.
- ISP: keep ports capability-specific. Do not create god interfaces that force no-op methods.
...
- Local-dev and deterministic-test may use in-memory/fake adapters, but tests must prove they follow the same port semantics as durable adapters.
...
- If a use case needs catalog/contract knowledge, introduce a port and put the contract-backed implementation in an adapter.
