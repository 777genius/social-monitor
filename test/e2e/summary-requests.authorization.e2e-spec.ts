...
describe('Summary request workspace authorization (e2e)', () => {
  ...
  it('requires a workspace role with summary request permission', async () => {
    const tenant = tenantId(deterministicTestUuid('tenant-summary-request-authorization-e2e'));
    const workspace = workspaceId(deterministicTestUuid('workspace-summary-request-authorization-e2e'));
