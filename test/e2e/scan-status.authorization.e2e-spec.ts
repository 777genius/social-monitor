...
describe('Scan status workspace authorization (e2e)', () => {
  ...
  it('requires any workspace role before reading scan job status', async () => {
    const tenant = tenantId(deterministicTestUuid('tenant-scan-status-authorization-e2e'));
    const workspace = workspaceId(deterministicTestUuid('workspace-scan-status-authorization-e2e'));
