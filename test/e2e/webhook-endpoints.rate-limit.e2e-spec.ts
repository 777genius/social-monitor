...
describe('Webhook endpoint public API rate limit (e2e)', () => {
  ...
  it('returns 429 after an API key exceeds the webhook management rate limit', async () => {
    const tenant = tenantId(deterministicTestUuid('tenant-webhook-rate-limit-e2e'));
    const workspace = workspaceId(deterministicTestUuid('workspace-webhook-rate-limit-e2e'));
